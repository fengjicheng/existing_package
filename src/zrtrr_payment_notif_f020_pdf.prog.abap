...
INCLUDE ZRTRN_PAYMENT_NOTIF_I00.
*INCLUDE rfkori00.
INCLUDE ZRTRN_PAYMENT_NOTIF_I01.
*INCLUDE rfkori02.
...
*ARAMETERS:     PSORT    LIKE RFPDO1-KORD10PS DEFAULT '1' NO-DISPLAY.
PARAMETERS:     rxopos   LIKE rfpdo1-kord10op DEFAULT ' '.
PARAMETERS:     rxopol   LIKE rfpdo1-kord10ol DEFAULT ' '.
PARAMETERS:     rxfaed   AS CHECKBOX.
...
* Begin of change by SRBOSE:ED2K904667: 20-Mar-2017
PARAMETERS cb_cust AS CHECKBOX.
PARAMETERS cb_doc  AS CHECKBOX.
* End of change by SRBOSE:ED2K904667: 20-Mar-2017
...
PARAMETERS:     rastbis2    LIKE rfpdo1-allgrogr DEFAULT '000'.
PARAMETERS:     rastbis3    LIKE rfpdo1-allgrogr DEFAULT '000'.
...
***<<<pdf-enabling
* Begin of change by SRBOSE:ED2K904667: 20-Mar-2017
*  save_repid       = 'RFKORD10_PDF'.
*  save_repid_alw   = 'RFKORD10'.
*  save_ftype       = '3'. "PDF
  save_repid       = 'ZRTRR_PAYMENT_NOTIF_F020_PDF'.
  save_repid_alw   = 'ZRTRR_PAYMENT_NOTIF_F020_PDF'.
  save_ftype       = '3'. "PDF
* End of change by SRBOSE:ED2K904667: 20-Mar-2017
...
* Process Currency Change
  perform PROC_CURR_CHANGE_ACTIV changing pcccheck.
...
*-------Daten extrahiert ?----------------------------------------------
  IF xextra IS INITIAL.
    ...
    DATA rfkord10_no_selection(1) TYPE C.
    rfkord10_no_selection = 'X'.
    EXPORT rfkord10_no_selection TO MEMORY ID 'RFKORD_NO_SELECTION'.
  ELSE.
*-----------------------------------------------------------------------
*       Sortierung
*-----------------------------------------------------------------------
...
*-----------------------------------------------------------------------
*       Kontoauszuege Formulardruck
*-----------------------------------------------------------------------

***<<<pdf-enabling
INCLUDE ZRTRN_PAYMENT_NOTIF_I02.
*  INCLUDE rfkori15pdf.
***>>>pdf-enabling
...
INCLUDE ZRTRN_PAYMENT_NOTIF_I03.
*  INCLUDE rfkori80.
...
INCLUDE ZRTRN_PAYMENT_NOTIF_I04.
*  INCLUDE rfkori90.
INCLUDE ZRTRN_PAYMENT_NOTIF_I06.
*  INCLUDE rfkori92.
...
*-----------------------------------------------------------------------
INCLUDE ZRTRN_PAYMENT_NOTIF_I05.
*  INCLUDE rfkori91.

*-----------------------------------------------------------------------
*       Allgemeine Unterroutinen für Messages und Protokoll
*-----------------------------------------------------------------------
INCLUDE ZRTRN_PAYMENT_NOTIF_I07.
*  INCLUDE rfkori93.
...
***<<<pdf-enabling
        ...
        PERFORM ausgabe_kontoauszug_pdf.    "RFKORI15PDF
...
FORM debitoren_daten.
  ...
  ENDIF.
  Perform Read_kna1_2.                                          "1996349
  Perform Read_knb1_2.                                          "1996349
...
*     PERFORM FORM_CLOSE.
      IF  NOT rsimul          IS INITIAL
      and not save_outputdone IS INITIAL.
...
*                         CHANGING i_final.
* PERFORM form_close_pdf.
*    PERFORM form_print_output USING t001f-fornr_pdf.
* End of change by SRBOSE:ED2K904667: 20-Mar-2017
...
FORM kreditoren_daten.
  ...
  ENDIF.
  Perform Read_lfa1_2.                                          "1996349
  Perform Read_lfb1_2.                                          "1996349
...
*        else.
      ...
      IF NOT rf140-vstid IS INITIAL.
        PERFORM verzugstage.
        IF NOT rxfaed IS INITIAL.
          IF hbsid-netdt GT save_faedt.
            DELETE hbsid.
            CONTINUE.
          ENDIF.
        ENDIF.
...
*        and hbsad-bstat ne 'S'
*        and SAVE_RXOPOL is initial.
*          save3_datum = datum01.
*        else.
          save3_datum = datum02.
*        endif.
...
FORM selection_ohne_bkorm.
  IF 'D' IN rkoart.
      Select * from Kna1 into table Hkna1                       "1996349
      where Kunnr in Rkonto
      order by primary key.                                     "1996349
      Select * from Knb1 into table Hknb1                       "1996349
      where Bukrs in Rbukrs
      and   Kunnr in Rkonto                                     "1996349
      order by primary key.                                     "1996349
      Loop at Hknb1 into Knb1.                                  "1996349
        At new Kunnr.                                           "1996349
           Read table Hkna1 into Kna1                           "1996349
           with key Kunnr = Knb1-Kunnr.                         "1996349
        Endat.                                                  "1996349
        CLEAR save_bbukr.
        IF NOT t048-xbukr IS INITIAL.
...
*         SAVE_BUKRS = KNB1-BUKRS.
        ...
        ENDIF.
     Endloop.                                                   "1996349
     If not Save_rxekvb is initial.                             "1996349
        Select * from Knb1                                      "1996349
        where Kunnr not in Rkonto                               "1996349
        and   Bukrs in Rbukrs                                   "1996349
        and   Ekvbd in Rkonto.                                  "1996349
           Insert Knb1 into table Hknb1.                        "1996349
           Select single * from Kna1                            "1996349
           where Kunnr = Knb1-Kunnr.                            "1996349
           Insert Kna1 into table Hkna1.                        "1996349
        Endselect.                                              "1996349
     Endif.                                                     "1996349
  Endif.
  If 'K' in Rkoart.
     Select * from Lfa1 into table Hlfa1                        "1996349
     where Lifnr in Rkonto
     order by primary key.                                      "1996349
     Select * from Lfb1 into table Hlfb1                        "1996349
     where Bukrs in Rbukrs
     And   Lifnr in Rkonto                                      "1996349
     order by primary key.                                      "1996349
     Loop at Hlfb1 into Lfb1.                                   "1996349
        At new Lifnr.                                           "1996349
           Read table Hlfa1 into Lfa1                           "1996349
           with key Lifnr = Lfb1-Lifnr.                         "1996349
        Endat.                                                  "1996349
        CLEAR save_bbukr.
        IF NOT t048-xbukr IS INITIAL.
...
FORM verband.
  IF hdkoart = 'D'.
    Clear: Hkna1, Hknb1, Filialen.                              "1996349
    Refresh Filialen.
    Loop at Hknb1 into *Knb1                                    "1996349
    where Bukrs = Save_bukrs                                    "1996349
    and   Ekvbd = Save_kunnr.                                   "1996349
...
*-----------------------------------------------------------------------
FORM zentrale.
  Clear: Hkna1, Hknb1, Hlfa1, Hlfb1, Filialen.                  "1996349
  REFRESH filialen.
* CLEAR HXDEZV.
