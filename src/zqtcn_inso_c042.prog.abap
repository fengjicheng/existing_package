*&---------------------------------------------------------------------*
*&  Include           ZQTCN_INSO_C042
*&---------------------------------------------------------------------*
*----------------------------------------------------------------------*
* PROGRAM NAME: ZQTCN_INSO_C042 (Enhancement Implementation)
* PROGRAM DESCRIPTION: Include for C042
* DEVELOPER: SOURDAS ( Sayantan Das )
* CREATION DATE:   17/01/2017
* OBJECT ID: C042
* TRANSPORT NUMBER(S):
*----------------------------------------------------------------------*
*** Local Structure declaration for XVBAK
TYPES: BEGIN OF lty_xvbak8. "Kopfdaten
...
TYPES: END OF lty_xvbak8.

TYPES: BEGIN OF d_flag_k8,
...
DATA: lst_xvbak8 TYPE lty_xvbak8,
      lst_flag8  TYPE d_flag_k8,
      lst_e1edk01_8     TYPE e1edk01. " IDoc: Document header general data

    CASE segment-segnam.
      WHEN 'E1EDK01'.
        lst_e1edk01_8 = segment-sdata.
        IF lst_e1edk01_8-bsart IS NOT INITIAL.
          CLEAR: lst_xvbak8.
          lst_xvbak8 = dxvbak.
          lst_xvbak8-bsark = lst_e1edk01_8-bsart.
          dxvbak = lst_xvbak8.

          lst_flag8 = dd_flag_k.
          lst_flag8-kbes = abap_true.
          dd_flag_k = lst_flag8.
        ENDIF.
*    	WHEN .
*    	WHEN OTHERS.
