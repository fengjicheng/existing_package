*&---------------------------------------------------------------------*
*&  Include           ZQTCN_INSO_C063
*&---------------------------------------------------------------------*
*----------------------------------------------------------------------*
* PROGRAM NAME: ZQTCN_INSO_C063 (Enhancement Implementation)
* PROGRAM DESCRIPTION: Include for C063
* DEVELOPER: SOURDAS ( Priyanka Mitra )
* CREATION DATE:   07/02/2017
* OBJECT ID: C063
* TRANSPORT NUMBER(S): ED2K904419
*----------------------------------------------------------------------*
*** Local Structure declaration for XVBAK
TYPES: BEGIN OF lty_xvbak_63. "Kopfdaten
...
TYPES: END OF lty_xvbak_63.

TYPES: BEGIN OF d_flag_k63,
...
DATA: lst_xvbak_63   TYPE lty_xvbak_63,
      lst_flag_63    TYPE d_flag_k63,
      lst_e1edk01_63 TYPE e1edk01. " IDoc: Document header general data

CASE segment-segnam.
  WHEN 'E1EDK01'.
    lst_e1edk01_63 = segment-sdata.
    IF lst_e1edk01_63-bsart IS NOT INITIAL.
      CLEAR: lst_xvbak_63.
      lst_xvbak_63 = dxvbak.
      lst_xvbak_63-bsark = lst_e1edk01_63-bsart.
      dxvbak = lst_xvbak_63.

      lst_flag_63 = dd_flag_k.
      lst_flag_63-kbes = abap_true.
      dd_flag_k = lst_flag_63.
    ENDIF.
*    	WHEN .
*    	WHEN OTHERS.
ENDCASE.
