...
FORM f_get_data_from_file.
  DATA: lst_cm_data TYPE ty_cm_sel,
        lst_cm      TYPE ty_cm_details.
  DATA:li_type TYPE truxs_t_text_data.
  FREE:i_data.
  CLEAR :v_path_fname,v_path_fname1,v_c107_path,v_file_path,v_dir.
  ...
  ELSE.
    ...
    CLEAR: i_cm_data_sel,lst_cm_data.
    OPEN DATASET p_infl FOR INPUT IN TEXT MODE ENCODING DEFAULT.
    IF sy-subrc = 0.
      DO.
        READ DATASET p_infl INTO lst_cm_data.
        IF sy-subrc = 0.
          APPEND lst_cm_data TO i_cm_data_sel.
          CLEAR lst_cm_data .
        ELSE.
          EXIT.
        ENDIF.
      ENDDO.
      SORT i_cm_data_sel BY kunnr vkorg vtweg matnr.
* Move the selected rows for background process
      LOOP AT i_cm_data_sel INTO lst_cm_data WHERE sel EQ c_x.
        MOVE-CORRESPONDING lst_cm_data TO lst_cm.
        APPEND lst_cm TO i_cm_data.
        CLEAR : lst_cm ,lst_cm_data.
      ENDLOOP.
    ELSE.
    ...
    IF lv_no_of_records IS NOT INITIAL AND lst_const-low IS NOT INITIAL.
      ...
      IF lv_no_of_records > lst_const-low.
        ...
        IF sy-subrc = 0.
          ...
          LOOP AT i_cm_data into lst_cm.
               MOVE-CORRESPONDING lst_cm to lst_cm_sel.
               APPEND lst_cm_sel to i_cm_data_sel.
               clear : lst_cm_sel, lst_cm.
          ENDLOOP.
* Mark the selected records as "X" in I_CM_DATA_SEL for background processing.
          LOOP AT li_rows INTO lst_rows.
            READ TABLE i_cm_data_sel ASSIGNING FIELD-SYMBOL(<fs_cm_sel>) INDEX lst_rows.
            IF <fs_cm_sel>  IS  ASSIGNED AND sy-subrc EQ 0.
              <fs_cm_sel>-sel = abap_true.
            ENDIF.
            clear lst_rows.
          ENDLOOP.
          UNASSIGN <fs_cm_sel>.
      ...
      ELSE.
        ...
        IF sy-subrc EQ 0.
          v_c107_path  =  lst_const1-low.
        ENDIF.
* Logic to send the Input file data to Aplication server path
        CONCATENATE lc_cmir sy-datum '_' sy-uzeit INTO lv_file.
      ...
      CONCATENATE 'Customer'(016)
        ...
        LOOP AT li_rows INTO lst_rows.
          ...
          IF <fs_cm_data>  IS  ASSIGNED AND sy-subrc EQ 0.
...
FORM f_submit_program_in_background USING p_file TYPE char80.
  ...
  DATA: lv_file TYPE string,
        lv_bjob TYPE char1.
  CLEAR : v_path_fname,v_c107_path.

  CONCATENATE text-001 sy-datum sy-uzeit INTO lv_file.
  v_path_fname =  lv_file.
  v_c107_path  = p_file.
  PERFORM f_get_file_path USING v_path_fname.
  CLEAR lv_file.
  ...
  OPEN DATASET lv_file FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.
  IF sy-subrc = 0.
    LOOP AT i_cm_data_sel INTO DATA(lst_cm_data).
      TRANSFER lst_cm_data TO lv_file.
      clear lst_cm_data.
    ENDLOOP.
  ENDIF.
  ...
  CLEAR:lv_jobname.
  CONCATENATE 'ZQTCR_CMIR_MASS_' sy-datum '_' sy-uzeit INTO lv_jobname.
  ...
  IF sy-subrc = 0.
    ...
    IF lv_bjob = abap_true.
      SUBMIT zqtcr_cmir_mass_load_02 WITH SELECTION-TABLE li_params
...
FORM f_error_success_log.
  ...
  IF sy-batch NE space.
    ...
    IF sy-subrc EQ 0.
      v_c107_path  =  lst_const-low.
    ENDIF.
  ENDIF.

  IF  i_sucess_rec IS NOT INITIAL.
    CONCATENATE lc_success sy-datum '_' sy-uzeit INTO DATA(lv_file).
    v_path_fname =  lv_file.
    PERFORM f_get_file_path USING v_path_fname.
    REPLACE ALL OCCURRENCES OF '/C107/in' IN v_file_path WITH '/C107/prc'.
    CLEAR lv_file.
    lv_file = v_file_path.
    CONDENSE  lv_file NO-GAPS.
    CLOSE DATASET lv_file.
  ...
  IF i_error_rec IS  NOT INITIAL.
    CONCATENATE lc_error sy-datum '_' sy-uzeit INTO DATA(lv_file_e).
    v_path_fname =  lv_file_e.
    PERFORM f_get_file_path USING v_path_fname.
    REPLACE ALL OCCURRENCES OF '/C107/in' IN v_file_path WITH '/C107/err'.
    CLEAR lv_file_e.
    lv_file_e = v_file_path.
    CONDENSE  lv_file NO-GAPS.
...
*&      Form  F_DELETE_FILE
*&---------------------------------------------------------------------*
*    Delete fie from Application server
*----------------------------------------------------------------------*
*  -->  p1        text
*  <--  p2        text
*----------------------------------------------------------------------*
...
*    IF sy-subrc EQ 4.
*      MESSAGE text-026  TYPE c_i DISPLAY LIKE c_i.
*    ENDIF.
*  ENDIF.
*ENDFORM.
