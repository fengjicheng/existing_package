...
FUNCTION ZQTC_ENTITLEMENT_STATUS_I0318N.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"     VALUE(IM_REVOKE_FLG) TYPE  XFELD OPTIONAL
*"  EXPORTING
...
* BOI by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
* To Revoked licence(s), Atleast one Revoked Item has to be passed
  CLEAR: lv_revoke_err.
  IF im_revoke_flg IS NOT INITIAL.
    lv_revoke_err = abap_true.
    LOOP AT im_ent_status TRANSPORTING NO FIELDS WHERE licence_start_date IS INITIAL.
      CLEAR lv_revoke_err.
      EXIT.
    ENDLOOP.
  ENDIF.
...
*  IF im_ent_status[] IS NOT INITIAL.
  IF im_ent_status[] IS NOT INITIAL AND lv_revoke_err IS INITIAL.
* EOI by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
...
* Check if the item entry exists in the VEDA table
                ...
                IF sy-subrc EQ 0.
* BOC by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
...
*                                                       i_agr_date_ind.
*                  ENDIF.
                  IF lst_veda-vabndat IS INITIAL AND
                     im_revoke_flg    IS INITIAL.
*               Subroutine to update agreement acceptance date
                    PERFORM f_pop_agreement_date USING lst_vbap
                                                       lst_ent_status_dummy
                                              CHANGING i_agr_date
                                                       i_agr_date_ind.
* Check if Revoked Item, then clear agreement acceptance date
                  ELSE. "IF lst_veda-vabndat IS INITIAL
                    IF lst_veda-vabndat IS NOT INITIAL AND
                       im_revoke_flg    IS NOT INITIAL AND
                       lst_ent_status_dummy-licence_start_date IS INITIAL. " Acceptance date
*               Subroutine to update agreement acceptance date to Blank
                      PERFORM f_pop_agreement_date USING lst_vbap
                                                         lst_ent_status_dummy
                                                CHANGING i_agr_date
                                                         i_agr_date_ind.
...
* EOC by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
                ELSE.
*  Check if acceptance date is already in system, then do not update that.
...
* Begin of Change by PBANDLAPAL on 06/16/2017 for ERP-2837
    PERFORM f_log_add USING c_err                              "Message Type
                            ...
                            space.                             "Message Variable 4
* BOI by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
    ELSEIF lv_revoke_err IS NOT INITIAL.  " IF im_ent_status[] IS INITIAL.
      CLEAR : lv_msg_type,
              lv_msgno.
      lv_msg_type = c_err.
      lv_msgno    = c_262.
...
*
      PERFORM f_log_add USING c_err                              "Message Type
                              ...
                              space                              "Message Variable 3
                              space.                             "Message Variable 4
    ENDIF. " IF im_ent_status[] IS INITIAL.
* EOI by NPALLA on 16-Dec-2021 for OCTM-44804:  ED2K925279
* End of Change by PBANDLAPAL on 06/16/2017 for ERP-2837
