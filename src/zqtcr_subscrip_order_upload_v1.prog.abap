*----------------------------------------------------------------------*
* Copy of Program "ZQTCR_SUBSCRIP_ORDER_UPLOAD" as part of OTCM-47267
* PROGRAM NAME: ZQTCR_SUBSCRIPTION_UPLOAD_V1 (Main Program)
...
*              - Upload File additional Validations                    *
*              - Logging in Staging Table (for E101)                   *
*----------------------------------------------------------------------*
REPORT zqtcr_subscrip_order_upload_v1 NO STANDARD PAGE HEADING
                                      MESSAGE-ID zqtc_r2.

*Begin of change by CHDAS: 28-Feb-2017:ED2K903417

*INCLUDE zqtcn_subs_ord_upload_rep_top IF FOUND. "constants declaration
INCLUDE zqtcn_subs_ord_upload_rep_top1 IF FOUND. "constants declaration

*INCLUDE zqtcn_subs_ord_upload_rep_sel IF FOUND. "selection screen
INCLUDE zqtcn_subs_ord_upload_rep_sel1 IF FOUND. "selection screen

*INCLUDE zqtcn_subs_ord_upload_rep_sub IF FOUND. "subroutines
INCLUDE zqtcn_subs_ord_upload_rep_sub1 IF FOUND. "subroutines
...
START-OF-SELECTION.
*---BOC NPALLA Staging Changes 09/01/2021 ED2K924398 E101  OTCM-47267
* To Use the Date Format as MM/DD/YYYY all through the program
SET COUNTRY 'US'.
*---EOC NPALLA Staging Changes 09/01/2021 ED2K924398 E101  OTCM-47267
* SOC by NPOLINA E101 Paths  ED2K913574 ERP7614
  IF i_const IS INITIAL.
    SELECT devid                      "Development ID
            ...
            INTO TABLE i_const
            WHERE ( devid   = c_e101 "c_devid  OTCM-47267
...
*--*Text ID
      READ TABLE i_const ASSIGNING FIELD-SYMBOL(<lst_e101>) WITH KEY devid = c_e101 "c_devid  OTCM-47267
                                                param1 = c_path
                                                BINARY SEARCH.
      IF sy-subrc EQ 0.
        v_e101 = <lst_e101>-low.
...
*---BOC NPALLA Staging Changes 09/01/2021 ED2K924398 E101  OTCM-47267
      LOOP AT i_const ASSIGNING <lst_e101> WHERE devid = c_e101. "c_devid.  OTCM-47267
        IF <lst_e101>-param1 = 'MVGR5'.
          APPEND INITIAL LINE TO ir_mvgr5 ASSIGNING FIELD-SYMBOL(<lfs_mvgr5>).
          <lfs_mvgr5>-sign = <lst_e101>-sign.
          <lfs_mvgr5>-opti = <lst_e101>-opti.
          <lfs_mvgr5>-low  = <lst_e101>-low.       " Material group 5
          <lfs_mvgr5>-high = <lst_e101>-high.      " Material group 5
        ENDIF.
        IF <lst_e101>-param1 = 'ROW' AND <lst_e101>-param2 = 'TEXT'.
          APPEND INITIAL LINE TO ir_row_txt ASSIGNING FIELD-SYMBOL(<lfs_row_txt>).
          <lfs_row_txt>-sign = <lst_e101>-sign.      "'I'.
          <lfs_row_txt>-opti = <lst_e101>-opti.      "'CP'.
          <lfs_row_txt>-low  = <lst_e101>-low.       " Row Text
          <lfs_row_txt>-high = <lst_e101>-high.      " Row Text
        ENDIF.
      ENDLOOP.
*---EOC NPALLA Staging Changes 09/01/2021 ED2K924398 E101  OTCM-47267
    ENDIF.
...
*                                            CHANGING i_final[]. "Input Data (table)
      PERFORM f_convert_new_subs_ord_excel1 USING    p_file     "File path
                                            CHANGING i_final[]. "Input Data (table)
*---EOC NPALLA Staging Changes 09/01/2021 ED2K924398 E101  OTCM-47267
***BOC BY SAYANDAS for CR-7614 on 24-AUG-2018 in ED2K913189
