...
* All the Subroutine has defined in this include.
* Here first creating PO of type ZNB , then performing goods Receipt
* and goods issue.
...
*------------------------------------------------------------------- *
* Revision No: ED1K909620,ED1K910059
* Reference No: RITM0079465
* Developer: Rajasekhar.T(RBTIRUMALA)
* Date: 25/04/2019
* Description: To Remove Duplicate IDoc Data in ALV output
*
*------------------------------------------------------------------- *
*------------------------------------------------------------------- *
* Revision No:  ED1K910554
* Reference No: INC0252114
* Developer: Bharani (BTIRUVATHU)
* Date: 10/07/2019
* Description: JDR records while processing still shows multiple
*              lines irrelevant error messages
*------------------------------------------------------------------- *
* Revision No:  ED1K910899
* Reference No: INC0258600
* Developer: Bharani (BTIRUVATHU)
* Date: 28/08/2019
* Description: JDR Records only get processed 2 at a time
*------------------------------------------------------------------- *
*-------------------------------------------------------------------
* REVISION HISTORY--------------------------------------------------
* REVISION NO:   ED1K911343
* REFERENCE NO:  INC0269637
* DEVELOPER:     Nikhilesh Palla (NPALLA)
* DATE:          2019-11-19
* DESCRIPTION:   JDR Pgm not processing all the records correctly
*-------------------------------------------------------------------

*&---------------------------------------------------------------------*
*&  Include           ZQTCN_INVENT_RECON_JDR_F01
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
...
* Local Constant Declaration
  CONSTANTS : lc_knttp_p    TYPE knttp      VALUE 'P',                       " Account Assignment Category
              ...
              lc_mesfct_gi  TYPE edi_mesfct VALUE 'GI',      " Message Function GR
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
         li_ccode_mat      TYPE TABLE OF ty_ccode_mat,
         lst_ccode_mat     TYPE ty_ccode_mat,
         li_poaccount      TYPE bapimepoaccount_tp,
         li_poaccountx     TYPE bapimepoaccountx_tp,
         lst_poaccount     TYPE bapimepoaccount,
         lst_poaccountx    TYPE bapimepoaccountx,
         li_poaddrdelv     TYPE bapimepoaddrdelivery_tp,
         lst_poaddrdelv    TYPE bapimepoaddrdelivery,
         lv_ser_no         TYPE dzekkn,
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
      SELECT lifnr,
             adrnr
             INTO TABLE @DATA(li_vend_addr)
             FROM lfa1
             FOR ALL ENTRIES IN @li_eord
             WHERE lifnr = @li_eord-lifnr.
      IF sy-subrc EQ 0.
        SORT li_vend_addr BY lifnr.
      ENDIF.
* EOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
    ENDIF. " IF sy-subrc EQ 0
  ENDIF. " IF li_inven_recon[] IS NOT INITIAL
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
* To get cost center details based on material and company code.
  DATA(li_inven_rec_tmp) = fp_i_inven_recon[].
  SORT li_inven_rec_tmp BY matnr werks.
  DELETE ADJACENT DUPLICATES FROM li_inven_rec_tmp COMPARING matnr werks.
  LOOP AT li_inven_rec_tmp INTO DATA(lst_inven_rec_tmp).

    READ TABLE li_cocd INTO DATA(lst_cocd) WITH KEY werks = lst_inven_rec_tmp-werks
                                                    BINARY SEARCH.
    IF sy-subrc EQ 0.
* Populate Company Code
      lst_ccode_mat-bukrs = lst_cocd-bwkey.
      lst_ccode_mat-matnr  = lst_inven_rec_tmp-matnr.
      APPEND lst_ccode_mat TO li_ccode_mat.
    ENDIF.
    CLEAR lst_ccode_mat.
  ENDLOOP.
  SORT li_ccode_mat BY matnr bukrs.
  DELETE ADJACENT DUPLICATES FROM li_ccode_mat COMPARING matnr bukrs.
  SELECT pbukr,
         fkstl,
         zzmpm
         INTO TABLE @DATA(li_prps)
         FROM prps
         FOR ALL ENTRIES IN @li_ccode_mat
         WHERE pbukr = @li_ccode_mat-bukrs AND
               zzmpm = @li_ccode_mat-matnr.
  IF sy-subrc EQ 0.
    SORT li_prps BY pbukr zzmpm.
    DELETE ADJACENT DUPLICATES FROM li_prps COMPARING pbukr zzmpm.
  ENDIF.
* EOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
...
*    READ TABLE li_cocd INTO DATA(lst_cocd) WITH KEY werks = lst_inven_recon-werks
    READ TABLE li_cocd INTO lst_cocd WITH KEY werks = lst_inven_recon-werks
...
*                                                               BINARY SEARCH.
*    IF sy-subrc EQ 0.
* Begin of Change:INC0269637:NPALLA:2019-11-19:ED1K911343
*    PERFORM f_check_posting_date_period
**                                       USING lst_inven_recon-zdate
*                                        CHANGING lir_date_range.
    IF lir_date_range[] IS INITIAL.
      PERFORM f_check_posting_date_period CHANGING lir_date_range.
    ENDIF.
* End of Change:INC0269637:NPALLA:2019-11-19:ED1K911343
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
    lst_item-item_cat    = p_itmcat.
    lst_itemx-item_cat   = abap_true.
    lst_item-acctasscat  = p_acctas.
...
* Populate Item Quantity
    lst_item-quantity = lst_inven_recon-zoffline.
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
* To Populate cost center
    READ TABLE li_prps INTO DATA(lst_prps) WITH KEY pbukr = lst_header-comp_code
                                                    zzmpm = lst_inven_recon-matnr
                                                    BINARY SEARCH.
    IF sy-subrc EQ 0.
      lst_poaccount-costcenter = lst_prps-fkstl.
      lst_poaccountx-costcenter = abap_true.
    ENDIF.
    lst_poaccount-po_item = lst_item-po_item.
    lv_ser_no = lv_ser_no + 1.
    lst_poaccount-serial_no = lv_ser_no.
    lst_poaccountx-po_item = lst_item-po_item.
    lst_poaccountx-serial_no = lv_ser_no.
    lst_poaccountx-po_itemx = abap_true.
    lst_poaccountx-serial_nox = abap_true.

    READ TABLE li_vend_addr INTO DATA(lst_addr_vend) WITH KEY lifnr = lst_header-vendor.
    IF sy-subrc EQ 0.
      lst_poaddrdelv-po_item = lst_item-po_item.
      lst_poaddrdelv-addr_no = lst_addr_vend-adrnr.
      APPEND lst_poaddrdelv TO li_poaddrdelv.
    ENDIF.
*    lst_poaccountx-
*    lst_poaccount-
* EOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
...
* BOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
    APPEND lst_poaccount TO li_poaccount.
    APPEND lst_poaccountx TO li_poaccountx.
* EOI by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
...
*    Create IDOC
      lst_edidc_po-mesfct  = lc_mesfct_gr.      " Message Function GR
...
*  Get the Material Document Number
      IF lv_idoc_num_gr IS NOT INITIAL .
        ...
        APPEND lst_alv TO i_alv.
* Begin of ADD:RITM0079465:RBTIRUMALA:25/04/2019:ED1K910059
        clear lst_alv.
* End of ADD:RITM0079465:RBTIRUMALA:25/04/2019:ED1K910059
...
* BOC by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
* the code is commented as part of defect ERP-6110 as GI process is not needed for JDR.
* Create IDOC for GI
****************************************************************************
**    To build the IDOC data structure
*      lv_segnam = lc_segnam.
...
*
**  Create IDOC for GI
*      lst_edidc_po-mesfct  = lc_mesfct_gi.     " Message Function GI
*      PERFORM f_create_idoc USING    lst_edidc_po
*                                     lc_evcode
*                            CHANGING li_edidd
**                                     lv_idoc_num_gi.
...
* Populate Table for updating ZQTC_INVEN_RECON table .
      ...
      IF lv_idoc_num_gr IS NOT INITIAL.
*         OR lv_idoc_num_gi IS NOT INITIAL.  " Comment by PBANDLAPAL on 26-Jan-2018 for ERP-6110: ED2K910553
...
*        lst_recon-zgi_mblnr = lv_mat_number_gi.
...
* Clear all the local Varible
    CLEAR: lv_purchase_order,
           ...
           lst_idoc_header,
           lst_idoc_item,
* BOC - BTIRUVATHU - INC0258600 - 2019/08/28 - ED1K910899
           lv_ser_no,
* EOC - BTIRUVATHU - INC0258600 - 2019/08/28 - ED1K910899
* BOC - BTIRUVATHU - INC0252114 - 2019/07/10 - ED1K910554
           li_poaccount[],
           li_poaccountx[],
           li_poaddrdelv[],
           lst_alv.
* EOC - BTIRUVATHU - INC0252114 - 2019/07/10 - ED1K910554
...
FORM f_set_edidc_po  CHANGING fp_lst_edidc_po TYPE edidc. " Control record (IDoc)
  ...
  CONSTANTS:
    ...
    lc_mescod_jdr TYPE edi_mescod VALUE 'JDR'.     " Message Variant JDR
...
* Get Logical System
  ...
  fp_lst_edidc_po-mescod = lc_mescod_jdr.     " Message Variant JDR
...
* Material DOC NO GR
  lv_col_pos = lv_col_pos + 1.
  PERFORM f_build_fcat USING lc_fld_mblnr  lc_tabname   lv_col_pos  'Material Doc No (GR)'(014)
                     CHANGING i_fcat.
