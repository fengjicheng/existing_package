*----------------------------------------------------------------------*
* Program name : ZQTCR_MED_ISS_R115_SUB_New (Inclde program)               *
...
**********************************************************************
  ...
  ELSEIF rb2 EQ 'X'.
    lv_exbb = lc_no.
  ENDIF.
  IF lv_exbb IS NOT INITIAL.
    ls_exbb-low = lv_exbb.
    ls_exbb-high = ''.
    ls_exbb-option = 'EQ'.
    ls_exbb-sign = 'I'.
    APPEND ls_exbb TO s_exbb.
  ENDIF.

***  IF c_exdb IS INITIAL.
  IF rb3 EQ 'X'.
    lv_exdb = lc_yes.
  ELSEIF rb4 EQ 'X'.
    lv_exdb = lc_no.
  ENDIF.
  IF lv_exdb IS NOT INITIAL.
    ls_exdb-low = lv_exdb.
    ls_exdb-high = ''.
    ls_exdb-option = 'EQ'.
    ls_exdb-sign = 'I'.
    APPEND ls_exdb TO s_exdb.
  ENDIF.

***  IF c_excb IS INITIAL.
  IF rb5 EQ 'X'.
    lv_excb = lc_yes.
  ELSEIF rb6 EQ 'X'.
    lv_excb = lc_no.
  ENDIF.
  IF lv_excb IS NOT INITIAL.
    ls_excb-low = lv_excb.
    ls_excb-high = ''.
    ls_excb-option = 'EQ'.
    ls_excb-sign = 'I'.
    APPEND ls_excb TO s_excb.
  ENDIF.

***  IF c_excc IS INITIAL.
  IF rb7 EQ 'X'.
    lv_excc = lc_yes.
  ELSEIF rb8 EQ 'X'.
    lv_excc = lc_no.
  ENDIF.
  IF lv_excc IS NOT INITIAL.
    ls_excc-low = lv_excc.
    ls_excc-high = ''.
    ls_excc-option = 'EQ'.
    ls_excc-sign = 'I'.
    APPEND ls_excc TO s_excc.
  ENDIF.

***  IF c_exro IS INITIAL.
  IF rb9 EQ 'X'.
    lv_exro = lc_yes.
  ELSEIF rb10 EQ 'X'.
    lv_exro = lc_no.
  ENDIF.
  IF lv_exro IS NOT INITIAL.
    ls_exro-low = lv_exro.
    ls_exro-high = ''.
    ls_exro-option = 'EQ'.
    ls_exro-sign = 'I'.
    APPEND ls_exro TO s_exro.
  ENDIF.

***  IF c_irel IS INITIAL.
  IF rb11 EQ 'X'.
    lv_irel = lc_yes.
  ELSEIF rb12 EQ 'X'.
    lv_irel = lc_no.
  ENDIF.
  IF lv_irel IS NOT INITIAL.
    ls_irel-low = lv_irel.
    ls_irel-high = ''.
    ls_irel-option = 'EQ'.
    ls_irel-sign = 'I'.
    APPEND ls_irel TO s_irel.
  ENDIF.
...
***  IF c_rbg IS INITIAL.
  ...
  ELSEIF rbg2 EQ 'X'.
    lv_iamt = lc_no.
  ENDIF.
  IF lv_iamt IS NOT INITIAL.
    ls_iamt-low = lv_iamt.
    ls_iamt-high = ''.
    ls_iamt-option = 'EQ'.
    ls_iamt-sign = 'I'.
    APPEND ls_iamt TO s_iamt.
  ENDIF.
**********************************************************************
...
*    AND unearned_flag IN @s_iamt.
  ...
  SELECT DISTINCT
         ...
         delivery_date,
         init_ship_date
  FROM zcds_mi_r115_rpt INTO TABLE @i_view
    WHERE
      journal IN @s_prod
      AND media_issue IN @s_issu
      AND publication_date IN @s_pbdt
...
* BOC by Lahiru on 02/22/2021 for OTCM-29592 with ED2K921929  *
    ...
    SELECT
      ...
      rejection_user,
      contract_soldto,
      contract_soldto_name,
      contract_shipto,
      contract_shipto_name,
      contract_shipto_ctry,
      invoice_number,
      invoice_item
     FROM zcds_con_r115
...
***            journal   = @i_view-journal.
    IF sy-subrc IS INITIAL.
      SORT i_contract_data BY contract
                              ...
                              media_issue.
    ENDIF.
***"* BOC by Krishna on 07/12/2021 for OTCM-45437 with ED2K924056  *
...
FORM f_set_pf_status USING rt_extab TYPE slis_t_extab.
  SET PF-STATUS 'ZPF_STATUS_R115_NEW'.
...
FORM f_build_final_output  USING fp_view TYPE zstqtc_mainvew_output_r115_new.

  APPEND INITIAL LINE TO i_alv_output ASSIGNING FIELD-SYMBOL(<lfs_alv_output>).
  <lfs_alv_output>-journal                = fp_view-journal.
  <lfs_alv_output>-media_issue            = fp_view-media_issue.
  <lfs_alv_output>-assignment             = fp_view-assignment.
  <lfs_alv_output>-contract               = fp_view-contract.
  <lfs_alv_output>-item                   = fp_view-item.
  <lfs_alv_output>-contract_reason        = fp_view-contract_reason.
  <lfs_alv_output>-item_category          = fp_view-item_category.
  <lfs_alv_output>-contract_start_date    = fp_view-contract_start_date.
  <lfs_alv_output>-contract_end_date      = fp_view-contract_end_date.
  <lfs_alv_output>-contract_qty           = fp_view-contract_qty.
  <lfs_alv_output>-plant                  = fp_view-plant.
  <lfs_alv_output>-mat_grp_5              = fp_view-mat_grp_5.
  <lfs_alv_output>-mat_grp_3              = fp_view-mat_grp_3.
  <lfs_alv_output>-net_value_item         = fp_view-net_value_item.
  <lfs_alv_output>-currency               = fp_view-currency.
  <lfs_alv_output>-billing_block_desc     = fp_view-billing_block_desc.
  <lfs_alv_output>-delivery_block_desc    = fp_view-delivery_block_desc.
  <lfs_alv_output>-overall_status_desc    = fp_view-overall_status_desc.
  <lfs_alv_output>-credit_status_desc     = fp_view-credit_status_desc.
  <lfs_alv_output>-cancel_reason_desc     = fp_view-cancel_reason_desc.
  <lfs_alv_output>-cancelled_orders_desc  = fp_view-cancelled_orders_desc.
  <lfs_alv_output>-reason_for_cancel_desc = fp_view-reason_for_cancel_desc.
  <lfs_alv_output>-acceptance_date        = fp_view-acceptance_date.
  <lfs_alv_output>-req_cancellation_date  = fp_view-req_cancellation_date.
  <lfs_alv_output>-price_grp_desc         = fp_view-price_grp_desc.
  <lfs_alv_output>-delivery_number        = fp_view-delivery_number.
  <lfs_alv_output>-delivery_item          = fp_view-delivery_item.
  <lfs_alv_output>-mat_doc                = fp_view-mat_doc.
  <lfs_alv_output>-mat_doc_item           = fp_view-mat_doc_item.
  <lfs_alv_output>-mat_doc_type           = fp_view-mat_doc_type.
  <lfs_alv_output>-mat_doc_year           = fp_view-mat_doc_year.
  <lfs_alv_output>-mat_doc_date           = fp_view-mat_doc_date.
  <lfs_alv_output>-po_number              = fp_view-po_number.
  <lfs_alv_output>-po_item                = fp_view-po_item.
  <lfs_alv_output>-po_type                = fp_view-po_type.
  <lfs_alv_output>-po_del_ind             = fp_view-po_del_ind.
  <lfs_alv_output>-po_info_rec            = fp_view-po_info_rec.
  <lfs_alv_output>-po_qty                 = fp_view-po_qty.
  <lfs_alv_output>-vendor                 = fp_view-vendor.
  <lfs_alv_output>-vendor_name            = fp_view-vendor_name.
  <lfs_alv_output>-value_per_issue        = fp_view-value_per_issue.
  <lfs_alv_output>-earned_amount          = fp_view-earned_amount.
  <lfs_alv_output>-balance                = fp_view-balance.
  <lfs_alv_output>-release_order_created  = fp_view-release_order_created.
  <lfs_alv_output>-release_order          = fp_view-release_order.
  <lfs_alv_output>-release_order_item     = fp_view-release_order_item.
  <lfs_alv_output>-doc_type               = fp_view-doc_type.
  <lfs_alv_output>-document_currency      = fp_view-document_currency.
  <lfs_alv_output>-purchase_requisition   = fp_view-purchase_requisition.
  <lfs_alv_output>-purchase_requisition_item = fp_view-purchase_requisition_item.
  <lfs_alv_output>-relevant_rar           = fp_view-relevant_rar .
  <lfs_alv_output>-sales_org              = fp_view-sales_org.
  <lfs_alv_output>-dist_channel           = fp_view-dist_channel.
  <lfs_alv_output>-division               = fp_view-division.
  <lfs_alv_output>-sales_office           = fp_view-sales_office.
  ...
  IF i_contract_data IS NOT INITIAL.
    ...
    IF sy-subrc = 0.
      <lfs_alv_output>-contract_created     = <lfs_contract_data>-contract_created.
      <lfs_alv_output>-rejection_user       = <lfs_contract_data>-rejection_user.
      <lfs_alv_output>-contract_soldto      = <lfs_contract_data>-contract_soldto.
      <lfs_alv_output>-contract_soldto_name = <lfs_contract_data>-contract_soldto_name.
      <lfs_alv_output>-contract_shipto      = <lfs_contract_data>-contract_shipto.
      <lfs_alv_output>-contract_shipto_name = <lfs_contract_data>-contract_shipto_name.
      <lfs_alv_output>-contract_shipto_ctry = <lfs_contract_data>-contract_shipto_ctry.
      <lfs_alv_output>-invoice_number       = <lfs_contract_data>-invoice_number.
    ENDIF.
  ENDIF.
***"* BOC by Krishna on 07/12/2021 for OTCM-45437 with ED2K924056  *
...
***                                                                    media_issue   = fp_view-media_issue BINARY SEARCH.
***    IF sy-subrc = 0.
***      <lfs_alv_output>-claims_order = <gfs_claim_orders>-claims_order.
***      <lfs_alv_output>-claims_item  = <gfs_claim_orders>-claims_item.
***    ENDIF.
***  ENDIF.
***
...
***                                                                               media_issue   = fp_view-media_issue BINARY SEARCH.
***    IF sy-subrc = 0.
***      <lfs_alv_output>-duplicate_count = <lfs_dup_orders>-row_count.
***    ENDIF.
***  ENDIF.
*** "EOC by Krishna on 07/12/2021 for OTCM-45437 with ED2K924056  *
ENDFORM.
...
FORM f_populate_mdata USING fp_lst_alv_row_mdata TYPE zstqtc_alvoutput_r115.
  ...
  IF sy-subrc = 0.
    ...
    <lfs_mdata>-publication_date   = <fs_view>-publication_date.
    <lfs_mdata>-actual_gi_date     = <fs_view>-actual_gi_date.
    <lfs_mdata>-material_status    = <fs_view>-material_status.
    <lfs_mdata>-distr_chain_status = <fs_view>-distr_chain_status.
    <lfs_mdata>-mi_ic_group        = <fs_view>-mi_ic_group.
    <lfs_mdata>-dist_chain_status  = <fs_view>-dist_chain_status.
    <lfs_mdata>-issue_number       = <fs_view>-issue_number.
    <lfs_mdata>-vol_copy_no        = <fs_view>-vol_copy_no.
    <lfs_mdata>-year_number        = <fs_view>-year_number.
    <lfs_mdata>-publication_type   = <fs_view>-publication_type.
    <lfs_mdata>-del_plant          = <fs_view>-del_plant.
    <lfs_mdata>-profit_center      = <fs_view>-profit_center.
    <lfs_mdata>-init_ship_date     = <fs_view>-init_ship_date.
    <lfs_mdata>-delivery_date      = <fs_view>-delivery_date.
    <lfs_mdata>-plan_status_desc   = <fs_view>-plan_status_desc.
    <lfs_mdata>-planned_ro_from    = <fs_view>-planned_ro_from.
    <lfs_mdata>-planned_ro_to      = <fs_view>-planned_ro_to.
...
***    ENDIF.
    ...
    IF <lfs_alv_output>-duplicate_count NE 0.
      ...
      IF li_duplicate_data IS NOT INITIAL.
        ...
        LOOP AT li_duplicate_data ASSIGNING <lfs_duplicate_data> FROM lv_dup_order_index. "#EC CI_NESTED
...
**** Submit Program
  ...
  IF sy-subrc = 0.
* BOC by Lahiru on 07/16/2021 with ED2K924056 *
*    SUBMIT zqtcr_media_issue_r115_new  WITH rb_fgr = rb_fgr                ##SUB_PAR
*                                   WITH rb_bgr = rb_bgr
...
*                                   WITH s_iamt IN s_iamt
*                                   WITH c_exbb IN s_exbb
*                                   WITH s_exdb IN s_exdb
*                                   WITH s_excb IN s_excb
*                                   WITH s_excc IN s_excc
*                                   WITH s_exro IN s_exro
*                                   WITH s_irel IN s_irel
...
FORM f_send_mail .
  ...
  CONSTANTS : lc_x(1)    TYPE c VALUE 'X',
              ...
              lc_count   TYPE rvari_vnam    VALUE 'ROW_CNT'.
...
****************************************************************************************
  DESCRIBE TABLE i_view LINES lv_line_count.

  READ TABLE i_constant INTO DATA(ls_constant) WITH KEY param1 = lc_count.
  IF sy-subrc = 0.
    IF lv_line_count LE ls_constant-low.
...
FORM zip_file .
  DATA:
    ...
    lv_content        TYPE bcsy_text,
    lst_content       TYPE so_text255,
    ...
    lv_time           TYPE sy-uzeit,
    lv_data_string    TYPE xstring, "declare string
    lv_string22       TYPE xstring.

* Creation of the Document Attachment
  LOOP AT i_xml_table ASSIGNING <gfs_xml>.
    lv_string22 = <gfs_xml>-data.
    CONCATENATE lv_data_string lv_string22 INTO lv_data_string IN BYTE MODE.
    CLEAR lv_string22.
  ENDLOOP.
*****************************************************************************************
...
***Xstring to binary
  ...
  CALL METHOD l_zipper->add
    EXPORTING
      name    = 'Media_issue_cockpit.xls' "filename
  ...
  " Mail Contents
  lst_text   = text-b02."'Dear User,'(078).
  ...
  APPEND lst_text TO li_text.
  CLEAR:lst_text.
  lst_text = text-b03."'Please find the attached JAPT Detail Report.'.
  APPEND lst_text TO li_text.

  lst_text =  text-b06.
  APPEND lst_text TO li_text.
  CLEAR:lst_text.
  lst_text = text-b04."'This is a system genarated email – please do not reply to it.'.
  APPEND lst_text TO li_text.
  ...
  APPEND lst_text TO li_text.
  CLEAR:lst_text.
  lst_text = text-b08.  "Wiley Technical team
  APPEND lst_text TO li_text.
  CLEAR:lst_text.
  ...
  lv_date = sy-datum.       " System date
  lv_time = sy-uzeit.       " System time
  CONCATENATE 'Media Issue Cockpit Report_' lv_date+4(2) lv_date+6(2) lv_date+0(4) INTO lv_subject.
  CONCATENATE lv_subject '_' lv_time INTO lv_subject.
  ...
  TRY .
      ...
      CALL METHOD cl_cam_address_bcs=>create_internet_address
        EXPORTING
          ...
          i_address_name   = 'no-reply@wiley.com'
        RECEIVING
          result           = lv_sender.
      ...
      lv_document_2 = cl_document_bcs=>create_document(
              i_type = lv_htm
              i_text = li_text
              i_subject = lv_subject )."'Media Issue Cockpit Report' ).   "#EC NOTEXT

      lv_document_2->add_attachment(
  i_attachment_type = 'ZIP'                                 "#EC NOTEXT
***  i_attachment_subject = 'Media Issue Cockpit Report file'  "#EC NOTEXT
          i_attachment_subject = lv_subject                 "#EC NOTEXT
  i_att_content_hex = lv_binary_content ).

      lv_mailto = v_receiver.    " 'ksrikanth@wiley.com'.
