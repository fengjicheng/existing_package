...
*  IF v_output_typ EQ v_outputyp_detail.
  PERFORM f_populate_layout_detail.
*  ENDIF. " IF v_output_typ EQ v_outputyp_detail
...
* Populate sales data in local structure.
  ...
  fp_st_vbco3-spras = 'EN'. "nast-spras. " Language Key
  fp_st_vbco3-vbeln = p_inv. "nast-objky+0(10). " Sales and Distribution Document Number
  fp_st_vbco3-posnr = '000010'. "nast-objky+10(6). " Item number of the SD document
...
* Local data declaration
  DATA: lst_sfpoutputparams TYPE sfpoutputparams,             " Form Processing Output Parameter
        ...
        lv_lines_form       TYPE i,                           " Lines_form of type Integers
...
****Local Constant declaration
  ...
  lv_form_name = lc_form_name."tnapr-sform.
...
**********EOC by MODUTTA on 20/07/2017 for archive****************************
  ...
  DATA(lv_lines) = lines( i_final ).
*  IF lst_sfpoutputparams-preview IS INITIAL
  IF lv_lines > 5000.
    PERFORM f_get_form_stitched.
  ELSE. " ELSE -> IF lst_sfpoutputparams-preview IS INITIAL
...
*       End   of ADD:ERP-6960:WROY:25-Mar-2018:ED2K911613
      ...
      ENDIF. " IF sy-subrc <> 0
    ENDIF. " IF sy-subrc <> 0
  ENDIF. " IF lst_sfpoutputparams-preview IS INITIAL
...
*       End   of ADD:ERP-6960:WROY:25-Mar-2018:ED2K911613
        ...
        lr_err_int          TYPE REF TO cx_fp_api_internal,   " Exception API (Internal)
        lr_text             TYPE string,
        li_final_form       TYPE ztqtc_item_detail_f024,
        lv_tabix            TYPE i,                           " Tabix of type Integers
        lv_lines_form       TYPE i,                           " Lines_form of type Integers
        lr_err_usg          TYPE REF TO cx_fp_api_usage.      " Exception API (Use)
...
****Local Constant declaration
  ...
  IF st_formoutput-pdf IS INITIAL.
    ...
    lst_sfpoutputparams-nodialog   = abap_true.
    lst_sfpoutputparams-connection = 'ADS'.
    lst_sfpoutputparams-bumode     = 'M'.
...
*          OTHERS                  = 4.
      ...
      LOOP AT i_final INTO DATA(lst_final).
        ...
        lv_tabix = lv_tabix + 1.
        APPEND lst_final1 TO li_final_form.
        IF lv_tabix = 15  AND lv_lines <> lv_tabix.
* Calling FM to generate detailed invoice
          CALL FUNCTION '/1BCDWB/SM00000125'
            EXPORTING
              /1bcdwb/docparams       = lst_sfpdocparams
              im_header               = st_header
              im_item                 = li_final_form
              im_xstring              = v_xstring
              im_footer               = v_footer
              im_remit_to             = v_remit_to
              im_v_tax                = v_tax
          ...
          CLEAR: li_final_form[],
                 lv_lines_form.
        ELSEIF lv_tabix > 15 AND lv_lines_form = 5000 AND lv_lines <> lv_tabix.
          CALL FUNCTION '/1BCDWB/SM00000126'       "/1BCDWB/SM00000089
            EXPORTING
              /1bcdwb/docparams       = lst_sfpdocparams
              im_header               = st_header
              im_item                 = li_final_form
          ...
          CLEAR:li_final_form[],
                lv_lines_form.
        ELSEIF lv_lines = lv_tabix.
          CALL FUNCTION '/1BCDWB/SM00000127'       "/1BCDWB/SM00000089
            EXPORTING
              /1bcdwb/docparams       = lst_sfpdocparams
              im_header               = st_header
              im_item                 = li_final_form
          ...
          CLEAR: li_final_form,
                 lv_tabix,
                 lv_lines_form.
        ENDIF. " IF lv_tabix = 15 AND lv_lines <> lv_tabix
      ENDLOOP. " LOOP AT i_final INTO DATA(lst_final)
...
*    lst_sfpoutputparams-getpdf = abap_true.
    ...
    lst_sfpoutputparams-nodialog = abap_true.
    lst_sfpoutputparams-preview = abap_false.
    DATA : p_conn   TYPE rfcdest,   " Logical Destination (Specified in Function Call)
           p_bum    TYPE fpbumode,  " Bundle Mode
           p_stitch TYPE fpboolean. " Data Type BOOLEAN
    p_conn = 'ADS'.
    p_bum = 'M'.
...
* Local data declaration
  DATA: lst_sfpoutputparams TYPE sfpoutputparams,                               " Form Processing Output Parameter
        ...
        lv_tabix            TYPE i,                                             " Tabix of type Integers
        lv_lines_form       TYPE i,                                             " Lines_form of type Integers
        lv_funcname1        TYPE funcname,                                      " Function name
        lv_funcname2        TYPE funcname,                                      " Function name
        lv_funcname3        TYPE funcname,                                      " Function name
        lv_form_name        TYPE fpname,                                        " Name of Form Object
        lr_err_rep          TYPE REF TO cx_fp_api_repository,                   " Exception API (Repository)
        lr_err_int          TYPE REF TO cx_fp_api_internal,                     " Exception API (Internal)
        lr_text             TYPE string,
        ...
        li_final_form       TYPE ztqtc_item_detail_f024,
        lv_form_name1       TYPE fpname VALUE 'ZQTC_FRM_INV_MNGD_DET_F024_HDR', " Name of Form Object
        lv_form_name2       TYPE fpname VALUE 'ZQTC_FRM_INV_MNGD_DET_F024_ITM', " Name of Form Object
        lv_form_name3       TYPE fpname VALUE 'ZQTC_FRM_INV_MNGD_DET_F024_LST', " Name of Form Object
        li_pdf_table        TYPE  tfpcontent.                                   " Form Processing: Content from XFT, XFD, PDF, and so on

  CALL FUNCTION 'FP_FUNCTION_MODULE_NAME'
    EXPORTING
      i_name     = lv_form_name1 "lc_form_name
    IMPORTING
      e_funcname = lv_funcname1.

  CALL FUNCTION 'FP_FUNCTION_MODULE_NAME'
    EXPORTING
      i_name     = lv_form_name2 "lc_form_name
    IMPORTING
      e_funcname = lv_funcname2.

  CALL FUNCTION 'FP_FUNCTION_MODULE_NAME'
    EXPORTING
      i_name     = lv_form_name3 "lc_form_name
    IMPORTING
      e_funcname = lv_funcname3.

  lst_sfpoutputparams-getpdf = 'M'.
  lst_sfpoutputparams-nodialog = abap_true.
  lst_sfpoutputparams-preview = abap_false.
  lst_sfpoutputparams-assemble  = 'S'.
  lst_sfpoutputparams-connection = 'ADS'.
  lst_sfpoutputparams-bumode   = 'M'. "this is bundle mode
* FM to open job for layout printing
  CALL FUNCTION 'FP_JOB_OPEN'
    CHANGING
      ie_outputparams = lst_sfpoutputparams
...
*   End   of ADD:ERP-6960:WROY:25-Mar-2018:ED2K911613
  ELSE. " ELSE -> IF sy-subrc <> 0
    lv_lines = lines( i_final ).
    DATA(lv_count_final) = lv_lines - 80.
    DATA(lv_end) = 80.
    LOOP AT i_final INTO DATA(lst_final) FROM 1 TO lv_end.
      DATA(lst_final1) = lst_final.
      APPEND lst_final1 TO li_final_form.
      CLEAR: lst_final1,lst_final.
    ENDLOOP. " LOOP AT i_final INTO DATA(lst_final) FROM 1 TO lv_end

* Calling FM to generate detailed invoice
    CALL FUNCTION lv_funcname1               "'/1BCDWB/SM00000125'
      EXPORTING
        /1bcdwb/docparams       = lst_sfpdocparams
        im_header               = st_header
        im_item                 = li_final_form
    ...
    IF sy-subrc = 0.
      CALL FUNCTION 'FP_GET_PDF_TABLE'
        IMPORTING
          e_pdf_table = li_pdf_table.
      CLEAR: li_final_form[].
    ENDIF. " IF sy-subrc = 0

    lv_lines_form = lv_count_final / 1000.
    DATA(lv_start) = lv_end + 1.
    lv_end = lv_end + 1000.
    DO lv_lines_form TIMES.
      LOOP AT i_final INTO lst_final FROM lv_start TO lv_end.
        lst_final1 = lst_final.
        APPEND lst_final1 TO li_final_form.
        CLEAR: lst_final1, lst_final.
      ENDLOOP. " LOOP AT i_final INTO lst_final FROM lv_start TO lv_end

      CALL FUNCTION lv_funcname2               "'/1BCDWB/SM00000126'       "/1BCDWB/SM00000089
        EXPORTING
          /1bcdwb/docparams       = lst_sfpdocparams
          im_header               = st_header
          im_item                 = li_final_form
    ...
    CALL FUNCTION lv_funcname3               "'/1BCDWB/SM00000127'       "/1BCDWB/SM00000089
      EXPORTING
        /1bcdwb/docparams       = lst_sfpdocparams
        im_header               = st_header
        im_item                 = li_final_form
...
*     FM to close layout after printing
    ...
    IF sy-subrc = 0.
      CALL FUNCTION 'FP_GET_PDF_TABLE'
        IMPORTING
          e_pdf_table = li_pdf_table.
      READ TABLE li_pdf_table INTO DATA(lv_pdf) INDEX 1.
      IF sy-subrc EQ 0.
        st_formoutput-pdf = lv_pdf.
        MESSAGE s246(zqtc_r2). " Email sent successfully
      ENDIF. " IF sy-subrc EQ 0
    ELSE. " ELSE -> IF sy-subrc = 0
*       Begin of ADD:ERP-6960:WROY:25-Mar-2018:ED2K911613
      CALL FUNCTION 'NAST_PROTOCOL_UPDATE'
