...
*-----------------------------------------------------------------------
FORM read_bkorm.
  If not Xerdt is initial.                                      "2207411
    SELECT        * FROM  bkorm
...
*       FORM READ_BSAD
*-----------------------------------------------------------------------
*       für Zahlungsmitteilungen und Wechselspesen
*-----------------------------------------------------------------------
FORM read_bsad.
  CLEAR   hbsad.
  REFRESH hbsad.
...
FORM read_bsec.
  ...
  IF sy-subrc NE 0.
    ...
    PERFORM message_append.
  ELSE.
*------- Alte Verschlüsselung vor 3.0E ? -> Neu verschlüsseln ----------
    IF bsec-empfg+5(1) = '0'.
...
*Begin of CVP Enablement
    if bseg-koart = 'K'.
    gs_bukrs_lifnr-LIFNR =   BSEG-LIFNR.
    gs_BUKRS_LIFNR-BUKRS  =  SAVE_BUKRS.
    append gs_bukrs_lifnr to  gt_bukrs_lifnr.
    clear: gs_bukrs_lifnr.
      ENDIF.
*End of CVP Enablement
...
FORM read_bseg_2.
  ...
  IF sy-subrc NE 0.
    ...
    PERFORM message_append.
  ELSE.
    READ TABLE lt_bseg INDEX 1 INTO bseg.
  ENDIF.
ENDFORM.                    "READ_BSEG_2
...
FORM read_bseg_arc.
  ...
  DATA: lt_doc_data TYPE as_t_tablebuffer,
        lw_doc_data TYPE astablebuffer,
        lt_abkpf    TYPE STANDARD TABLE OF abkpf,
...
*            OTHERS                      = 7.
  ...
  lw_doc_data-tabname = 'BKPF'.
  APPEND lw_doc_data TO lt_doc_data.
  lw_doc_data-tabname = 'BSEG'.
  APPEND lw_doc_data TO lt_doc_data.
  CLEAR: lw_doc_data.

  CALL FUNCTION 'FI_DOCUMNT_ARCH_READ_SGL_BELNR'
    EXPORTING
      i_bukrs            = save2_bukrs
      i_belnr            = save2_belnr
      i_gjahr            = save2_gjahr
  ...
  READ TABLE lt_doc_data INTO lw_doc_data
       WITH KEY tabname = 'BKPF'.
  ASSIGN lw_doc_data-tabref->* TO <lt_records>.
  IF sy-subrc = 0.
    lt_abkpf = <lt_records>.
    habkpf[] = lt_abkpf[].
  ENDIF.
...
FORM read_kna1_2.
  ...
  with key Kunnr = Save2_kunnr.                                 "1996349
  If sy-subrc ne 0.                                             "1996349
...
FORM read_knb1_2.
  CLEAR *knb1.
  Read table Hknb1 into *Knb1                                   "1996349
  with key Kunnr = Save2_kunnr                                  "1996349
           Bukrs = Save2_bukrs.                                 "1996349
  If sy-subrc ne 0.                                             "1996349
...
FORM read_lfa1_2.
  ...
  with key Lifnr = Save2_lifnr.                                 "1996349
  If sy-subrc ne 0.                                             "1996349
...
FORM read_lfb1_2.
  CLEAR *lfb1.
  Read table Hlfb1 into *Lfb1                                   "1996349
  with key Lifnr = Save2_lifnr                                  "1996349
           Bukrs = Save2_bukrs.                                 "1996349
  If sy-subrc ne 0.                                             "1996349
...
***<<<pdf_enabling
  ...
  DATA save_koart TYPE rfkord_ftype.
  CLEAR: save_fm_name, save_koart.
***>>>pdf-enabling

  CASE save_ftype.
    WHEN space.             "SAPscript
      save_koart = space.
    WHEN '3'.               "PDF
      save_koart = '3'.     "PDF
  ENDCASE.
          ...
          ELSE.
            ...
            TRY.
                CALL FUNCTION 'FP_FUNCTION_MODULE_NAME'
                  EXPORTING
                    i_name     = t001f-fornr_pdf
                  IMPORTING
                    e_funcname = fm_name.
              CATCH cx_fp_api.
            ENDTRY.

            save_fm_name = fm_name.

            IF save_fm_name IS INITIAL.
              MESSAGE e848 WITH t001f-fornr_pdf save_bukrs
                 save_repid save_forid.
            ENDIF.
****************start of pdf changes by c5112660***********
             if t001f-fornr_pdf is not initial.
             " to get the data selected for interface
             xopen = 'Y'.
             endif.
...
FORM read_partnerbank.
  ...
  DATA: lt_knbk TYPE TABLE OF knbk,
        lt_lfbk TYPE TABLE OF lfbk.                             "1755648
  CLEAR: *bnka, save_iban.
  CASE save_koart.
    WHEN 'D'.
      SELECT * FROM knbk INTO TABLE lt_knbk
        WHERE kunnr = save_kunnr
        AND   bvtyp = save_bvtyp.
      LOOP AT lt_knbk INTO knbk.                                "1755648
        CLEAR save_bankl.
        CLEAR save_bankn.
        CLEAR save_banks.
        save_bankl = knbk-bankl.                                "1755648
        save_bankn = knbk-bankn.                                "1755648
        save_banks = knbk-banks.                                "1755648
        PERFORM read_bnka_2.
        CALL FUNCTION 'READ_IBAN'                               "1799437
            EXPORTING
              i_banks        = save_banks
              i_bankl        = save_bankl
              i_bankn        = save_bankn
              i_bkont        = knbk-bkont                       "1755648
            IMPORTING
              e_iban         = save_iban
            EXCEPTIONS
              iban_not_found = 0.                               "1799437
        EXIT.
      ENDLOOP.
      IF sy-subrc NE 0.
        CLEAR fimsg.
    ...
    WHEN 'K'.
      SELECT * FROM lfbk INTO TABLE lt_lfbk
        WHERE lifnr = save_lifnr
        AND   bvtyp = save_bvtyp.
      LOOP AT lt_lfbk INTO lfbk.                                "1755648
        CLEAR save_bankl.
        CLEAR save_bankn.
        CLEAR save_banks.
        save_bankl = lfbk-bankl.                                "1755648
        save_bankn = lfbk-bankn.                                "1755648
        save_banks = lfbk-banks.                                "1755648
        PERFORM read_bnka_2.
        CALL FUNCTION 'READ_IBAN'                               "1799437
            EXPORTING
              i_banks        = save_banks
...
*       FORM READ_TAX_DATA
*-----------------------------------------------------------------------
*       Lesen der Steuerarten, Prozentsätze und Abzugsfähigkeit
*       als Steuerbasis wird hier immer 100000 übergeben
*-----------------------------------------------------------------------
FORM read_tax_data.
  CLEAR   htax.
  REFRESH htax.
...
*-----------------------------------------------------------------------
*       Lesen der Buchungsschlüssel für Ein- und Auszahlungen
*-----------------------------------------------------------------------
FORM read_zahlungs_bschl.
