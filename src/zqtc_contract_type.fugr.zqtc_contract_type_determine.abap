FUNCTION zqtc_contract_type_determine.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(IM_OBJECTID) TYPE  ZDEVID
*"     VALUE(IM_PARAM1) TYPE  RVARI_VNAM
*"     VALUE(IM_AUART) TYPE  AUART
*"  EXPORTING
*"     VALUE(EX_ACTIVE_FLAG) TYPE  CHAR1
*"----------------------------------------------------------------------
*----------------------------------------------------------------------*
* Functional Module NAME: ZQTC_CONTRACT_TYPE_DETERMINE
* PROGRAM DESCRIPTION: This FM is used to retrieve the sales doc types
* fro ZCACONSTANT table for DevIDs E164&E107 for auart.
* DEVELOPER: Kiran Jagana
* CREATION DATE:   05/12/2018
* OBJECT ID: CR7848
* TRANSPORT NUMBER(S): ED2K913989
*----------------------------------------------------------------------*
* REVISION HISTORY--------------------------------------------------------*
* REVISION NO  :
* REFERENCE NO :
...
TYPES:
 ...
 DATA :
    li_constant TYPE STANDARD TABLE OF
     ...
     lr_auart    TYPE ltt_auart,              " Doc type range table
    lst_auart   TYPE lty_auart.              " Doc type
