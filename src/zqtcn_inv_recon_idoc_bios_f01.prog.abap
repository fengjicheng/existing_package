*-------------------------------------------------------------------
* PROGRAM NAME: ZQTCN_INVENT_RECON_BIOS_F01
* PROGRAM DESCRIPTION: Inventory Reconcilliation
* All the Subroutine has defined this include.
...
*---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&  Include           ZQTCN_INVENT_RECON_BIOS_F01
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
...
FORM f_get_zqtc_inven_recon CHANGING fp_i_inven_recon TYPE tt_inven_recon.
  ...
  WHERE zadjtyp = p_adjtyp
   AND  zadjqty NE space
   AND  zgi_docnum = space.
  IF sy-subrc EQ 0.
...
* Local Constant Declaration
  CONSTANTS : lc_knttp_p    TYPE knttp      VALUE 'P',                       " Account Assignment Category
              ...
              lc_msg_no_236 TYPE edi_stamno VALUE '236',                     " Status message number
...
* Get the company code from T001W table .
  IF li_inven_recon[] IS NOT INITIAL.
    SELECT werks, " Plant
*          Begin of DEL:ERP-7308:WROY:29-Mar-2018:ED2K911689
*          bwkey  " Valuation Area
*          End   of DEL:ERP-7308:WROY:29-Mar-2018:ED2K911689
*          Begin of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
...
* Begin of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
  ...
  DELETE ADJACENT DUPLICATES FROM li_inven_recon
               COMPARING matnr werks.
  LOOP AT li_inven_recon ASSIGNING FIELD-SYMBOL(<lst_inven_recon>).
    READ TABLE li_cocd ASSIGNING FIELD-SYMBOL(<lst_cocd>)
         WITH KEY werks = <lst_inven_recon>-werks
         BINARY SEARCH.
    IF sy-subrc EQ 0.
*     Populate Company Code
      APPEND INITIAL LINE TO li_ccode_mat ASSIGNING FIELD-SYMBOL(<lst_ccode_mat>).
      <lst_ccode_mat>-bukrs = <lst_cocd>-bukrs.
      <lst_ccode_mat>-matnr = <lst_inven_recon>-matnr.
    ENDIF.
  ENDLOOP.
  IF li_ccode_mat[] IS NOT INITIAL.
    SORT li_ccode_mat BY matnr bukrs.
    DELETE ADJACENT DUPLICATES FROM li_ccode_mat
               COMPARING matnr bukrs.
*   Fetch WBS (Work Breakdown Structure) Element Master Data
    SELECT pspnr,
           pbukr,
           fkstl,
           zzmpm
     ...
     WHERE pbukr EQ @li_ccode_mat-bukrs
       AND zzmpm EQ @li_ccode_mat-matnr.
    IF sy-subrc EQ 0.
...
* For one material their will be only one PO.
  LOOP AT fp_i_inven_recon INTO DATA(lst_inven_recon).
    ...
    IF lst_inven_recon-zadjqty IS INITIAL.
      lst_alv-light = lc_red. "red traffic light
      lst_alv-msg_id = lc_msg_id.
      lst_alv-msg_no = lc_msg_no_236.
      MESSAGE ID lc_msg_id
      TYPE       lc_msg_ty
      NUMBER     lc_msg_no_236
      INTO       lst_alv-message.
      CLEAR lv_seqno.
      lv_seqno = lst_inven_recon-zseqno.
      CONDENSE lv_seqno.
      CONCATENATE lst_alv-message lst_inven_recon-zdate lv_seqno
                   INTO lst_alv-message SEPARATED BY space.
      APPEND lst_alv TO i_alv.
      CLEAR lst_alv.
      CONTINUE.
    ENDIF.
...
*    To build the IDOC data structure
    ...
    lst_idoc_header-pstng_date = lst_header-doc_date.
    lst_idoc_header-doc_date   = lst_header-doc_date.
    lst_idoc_header-ref_doc_no = lst_inven_recon-zsource.
...
*                                                     BINARY SEARCH .
    IF sy-subrc EQ 0.
      CLEAR lv_meins.
      lv_meins = lst_mara-meins.

    ENDIF. " IF sy-subrc EQ 0

*   Begin of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
    READ TABLE li_cocd ASSIGNING <lst_cocd>
         WITH KEY werks = lst_inven_recon-werks
         BINARY SEARCH.
    IF sy-subrc EQ 0.
      READ TABLE li_prps INTO DATA(lst_prps)
           WITH KEY pbukr = <lst_cocd>-bukrs
...
*      lst_recon-mblnr = lv_mat_number_gi.
...
FORM f_set_edidc_po  CHANGING fp_lst_edidc_po TYPE edidc. " Control record (IDoc)
  ...
  CONSTANTS:
    ...
    lc_mesfct_gi  TYPE edi_mesfct VALUE 'GI',      " Message Function GI
    lc_mescod_bis TYPE edi_mescod VALUE 'BIS'.     " Message Variant BIS
...
* Get Logical System
  ...
  fp_lst_edidc_po-mescod = lc_mescod_bis.     " Message Variant BIS
  fp_lst_edidc_po-mesfct = lc_mesfct_gi.      " Message function GI
...
FORM f_populate_item  USING    fp_lst_item          TYPE bapimepoitem             " Purchase Order Item
                               fp_lv_meins          TYPE meins                    " Purchasing Document Number
                               fp_lst_inven_recon   TYPE zqtc_inven_recon         " Table for Inventory Reconciliation Data
*                              Begin of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
                               fp_lst_prps_fkstl    TYPE kostl                    " Cost Center
*                              End   of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
  ...
  fp_lst_idoc_item-material  = fp_lst_inven_recon-matnr.
  fp_lst_idoc_item-plant     = fp_lst_inven_recon-werks.
  fp_lst_idoc_item-stge_loc  = p_lgort.
* Begin of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
  fp_lst_idoc_item-costcenter = fp_lst_prps_fkstl.  " Cost Center
* End   of ADD:ERP-7308:WROY:29-Mar-2018:ED2K911689
  fp_lst_idoc_item-entry_qnt = fp_lst_inven_recon-zadjqty.
  CONDENSE fp_lst_idoc_item-entry_qnt.
  fp_lst_idoc_item-entry_uom = fp_lv_meins.
