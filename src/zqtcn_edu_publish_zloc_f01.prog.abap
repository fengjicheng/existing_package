...
* Call function module to generate LOC form
      CALL FUNCTION lv_funcname "'/1BCDWB/SM00000157'
        EXPORTING
          ...
          im_hdr             = li_hdr_loc
          im_text            = v_trscript     "so10 text script
          im_bmp             = v_bmp
        IMPORTING
...
***Material Sales text logic
  SELECT SINGLE vbeln,vkorg,vtweg,vkbur                             " Fetch Sales org Dis channel
          FROM vbak
          INTO @DATA(lw_vbak)
          WHERE vbeln = @nast-objky+0(10).
  IF sy-subrc EQ 0.
...
*---Get the university name
          SELECT SINGLE name1 INTO li_hdr_loc-univ_name FROM lfa1 WHERE lifnr = lv_vendorno.
          SELECT SINGLE partner,idnumber FROM but0id
                                  INTO @DATA(lw_but0id)
                                  WHERE type = @c_zstuid
                                   AND  institute = @li_hdr_loc-univ_id.   "Partner University Number
          IF sy-subrc = 0.
            li_hdr_loc-idnumber = lw_but0id-idnumber.
            CONCATENATE li_hdr_loc-univ_name 'ID:' INTO li_hdr_loc-idnumber_desc.
          ENDIF.
        ENDIF.
...
** Read lc_credit_typ graduateCredit or continuingEducation
        ...
        IF sy-subrc = 0.
          ...
          IF li_hdr_loc-credit_type = 'ASPDP'.
            li_hdr_loc-univ_name = 'ASPDP / NYCDOE'.
          ENDIF.
        ENDIF.
**** Read Grade
        li_hdr_loc-grade = lw_vbap-mvgr3.
**   logic to read the template
        READ TABLE lt_allocvalueschar INTO lst_allocvalueschar WITH KEY charact = lc_loc_template.
        IF sy-subrc = 0.
          DATA(lv_template) = lst_allocvalueschar-value_char.
        ENDIF.
** Product code
        READ TABLE lt_allocvalueschar INTO lst_allocvalueschar WITH KEY charact = lc_product_code.
        IF sy-subrc = 0.
          li_hdr_loc-prod_code = lst_allocvalueschar-value_char.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDIF.
...
** Populate postal code
  ...
  ENDIF.
  REFRESH:lt_lines.
*** Logic to read the SO10 text
** Read the SO10 text based on the university number
  CONCATENATE lc_zqtcn_loc v_output_typ li_hdr_loc-univ_id
  INTO lv_name SEPARATED BY lc_under.
  PERFORM read_text USING lc_st c_e lv_name lc_object_t
              CHANGING lt_lines
                       lv_text.
  li_hdr_loc-trans_name = lv_name.
  LOOP AT lt_lines ASSIGNING FIELD-SYMBOL(<lw_lines>).
    REPLACE ALL OCCURRENCES OF '&st_name&' IN <lw_lines>-tdline
    WITH li_hdr_loc-st_name.
    REPLACE ALL OCCURRENCES OF '&PROD_CODE&' IN <lw_lines>-tdline
    WITH li_hdr_loc-prod_code.
    REPLACE ALL OCCURRENCES OF '&COURSE_TIT&' IN <lw_lines>-tdline
    WITH li_hdr_loc-sales_text.
    REPLACE ALL OCCURRENCES OF '&GRADE_DATE&' IN <lw_lines>-tdline
    WITH li_hdr_loc-grade_date.
    REPLACE ALL OCCURRENCES OF '&GRADE&' IN <lw_lines>-tdline
    WITH li_hdr_loc-grade.
    REPLACE ALL OCCURRENCES OF '&CREDIT_DIS&' IN <lw_lines>-tdline
    WITH li_hdr_loc-credit_dis.
    REPLACE ALL OCCURRENCES OF '&UNIV_NAME&' IN <lw_lines>-tdline
     WITH li_hdr_loc-univ_name.
    CONCATENATE v_trscript <lw_lines>-tdline INTO v_trscript.
  ENDLOOP.
  CONCATENATE '<body xmlns="http://www.w3.org/1999/xhtml" xmlns:xfa="http://www.xfa.org/schema/xfa-data/1.0/">'
  v_trscript '</body>' INTO v_trscript.
***********End logic************
****Logic to read the footer text
  IF lw_vbak-vkbur IS NOT INITIAL.
    SELECT SINGLE * FROM tvbur INTO @DATA(lw_tvbur)
                             WHERE vkbur = @lw_vbak-vkbur.
    IF sy-subrc = 0.
      SELECT SINGLE * FROM adrc INTO @DATA(lw_sales)
                        WHERE addrnumber = @lw_tvbur-adrnr.
      IF sy-subrc = 0.
        CONCATENATE lw_sales-house_num1 lw_sales-street '|'
        lw_sales-city1 INTO li_hdr_loc-footer_text SEPARATED BY space.
        CONCATENATE li_hdr_loc-footer_text lc_com INTO li_hdr_loc-footer_text.
        CONCATENATE li_hdr_loc-footer_text lw_sales-region
        lw_sales-post_code1 '|' lw_sales-tel_number INTO li_hdr_loc-footer_text
        SEPARATED BY space.
      ENDIF.
    ENDIF.
  ENDIF.
*  CONCATENATE lc_zqtcn_loc v_output_typ 'FOOTER'
*  INTO li_hdr_loc-footer_text SEPARATED BY lc_under.
*--Logo
...
* Local constant declaration
  CONSTANTS : lc_logo_name TYPE tdobname   VALUE 'ZWILEY_LOGO_AC_256', " Name
