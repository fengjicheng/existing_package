FUNCTION zscm_create_inforecord_e244.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* Local Data
  ...
  IF <lv_preq_date> IS ASSIGNED AND
    ...
    IF <lfi_outtab> IS ASSIGNED AND <lfi_outtab> IS NOT INITIAL.
      ...
      IF sy-subrc <> 0.
        ...
        EXIT.
      ENDIF.
      " Call transaction for Info Record creation
  ...
  ELSE.
    MESSAGE '"PR-InfoRecord" is only applicable for Purchase Req. Date'(003) TYPE 'I'.
    EXIT.
  ENDIF.
