*&---------------------------------------------------------------------*
*&  Include           ZQTCN_CHG_MAT_ITEM_TEXT_SUB
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form  F_SEL_DYNAMICS
*&---------------------------------------------------------------------*
...
*----------------------------------------------------------------------*
FORM f_initialization .
  REFRESH:li_mara,li_tline,li_alv_disp_screen,li_excel_file,
  ...
  IF rb_input = c_x AND rb_file NE c_x.
    LOOP AT SCREEN.
      IF screen-group1 = 'M4'.
        ...
        MODIFY SCREEN.
      ENDIF.
      IF screen-group1 = 'M3' OR screen-group1 = 'M5' OR screen-group1 = 'M6' OR screen-group1 = 'M7' OR
        screen-group1 = 'M8' OR screen-group1 = 'M9'.
        screen-input = screen-active = screen-output = 0.
        MODIFY SCREEN.
      ENDIF.
    ENDLOOP.
  ENDIF.
  IF rb_input NE c_x AND rb_file = c_x.
    LOOP AT SCREEN.
      IF screen-group1 = 'M4'.
        screen-input = screen-active = screen-output = 0.
        MODIFY SCREEN.
...
FORM f_get_data.
  IF li_excel_file[] IS NOT INITIAL.
    IF r_b_txt IS NOT INITIAL.
      SELECT matnr  FROM mara
             INTO TABLE li_mara
             FOR ALL ENTRIES IN li_excel_file
             WHERE matnr = li_excel_file-matnr.
    ENDIF.
    IF r_s_txt IS NOT INITIAL.
      SELECT matnr vkorg vtweg FROM mvke
             INTO TABLE li_mara
             FOR ALL ENTRIES IN li_excel_file
             WHERE matnr = li_excel_file-matnr
             AND   vkorg = li_excel_file-vkorg
             AND   vtweg = li_excel_file-vtweg.
    ENDIF.
  ENDIF.
ENDFORM.
*&---------------------------------------------------------------------*
...
* Local data ----------------------------------------------------------
  ...
  CLEAR g_path_fname.
  IF sy-sysid = 'EQ1'.
    p_a_file = '/intf/tib/uat/jfds/I0255_label/in/'.
  ELSEIF   sy-sysid = 'EP1'.
    p_a_file = '/intf/tib/prd/jfds/I0255_label/in/'.
  ENDIF.
  ...
  LOOP AT li_excel_file INTO DATA(lst_file).
    ...
    CONCATENATE lst_file-matnr lst_file-lang   lst_file-eal
           INTO lv_data
      SEPARATED BY lv_tab.
...
* Local constant declaration
  CONSTANTS: lc_job_name   TYPE btcjob VALUE 'ZMAT_TEXT_UPD'. " Background job name
  CLEAR lv_job_name.
...
*     sdlstrttm        = sy-uzeit
  ...
  IF sy-subrc = 0.
    ...
    p_job   = lv_job_name.
    SUBMIT zqtcn_chg_mat_item_text_batch
...
FORM f_sales_ord_text_upd.
  ...
  LOOP AT li_excel_file INTO st_excel_file.
    st_alv_disp_screen-matnr = st_excel_file-matnr.
    st_alv_disp_screen-lang = st_excel_file-lang.
    READ TABLE li_mara INTO DATA(lst_mara) WITH KEY matnr = st_excel_file-matnr
                                           BINARY SEARCH.
    IF sy-subrc EQ 0.
      ...
      IF r_b_txt IS NOT INITIAL.
        st_header-tdname = lst_mara-matnr.
      ENDIF.
      IF r_s_txt IS NOT INITIAL.
        CONCATENATE lst_mara-matnr lst_mara-vkorg lst_mara-vtweg INTO
        st_header-tdname.
        st_alv_disp_screen-vkorg = lst_mara-vkorg.
        st_alv_disp_screen-vtweg = lst_mara-vtweg.
      ENDIF.
      st_header-tdid = p_id.
      st_header-tdspras = st_excel_file-lang.
...
* Implement suitable error handling here
                ...
                st_alv_disp_screen-status_message = 'Material text is updated successfully'(012).
                APPEND st_alv_disp_screen TO li_alv_disp_screen.
                CLEAR st_alv_disp_screen.
              ELSE.
                st_alv_disp_screen-new_eal = st_excel_file-eal.
                st_alv_disp_screen-status = 'Error'(013).
                st_alv_disp_screen-status_message = 'Material Text not updated to given sales document and item'(014).
                APPEND st_alv_disp_screen TO li_alv_disp_screen.
                CLEAR st_alv_disp_screen.
              ENDIF.
            ELSE.
              st_alv_disp_screen-status = 'Error'.
              st_alv_disp_screen-status_message = 'Material text is not existed in excel file'.
              APPEND st_alv_disp_screen TO li_alv_disp_screen.
              CLEAR st_alv_disp_screen.
            ENDIF.
          ENDIF.
        ELSE.
          ...
          st_alv_disp_screen-status_message =  'OLD and NEW Material texts are same'.
          APPEND st_alv_disp_screen TO li_alv_disp_screen.
          CLEAR st_alv_disp_screen.
        ENDIF.
      ELSE.
...
* Implement suitable error handling here
            ...
            st_alv_disp_screen-new_eal = st_excel_file-eal.
            st_alv_disp_screen-status =  'Success'.
            st_alv_disp_screen-status_message = 'Material text is updated successfully'.
            APPEND st_alv_disp_screen TO li_alv_disp_screen.
            CLEAR st_alv_disp_screen.
          ELSE.
            st_alv_disp_screen-new_eal = st_excel_file-eal.
            st_alv_disp_screen-status = 'Error'.
            st_alv_disp_screen-status_message = 'Material Text not updated to given sales document and item'.
            APPEND st_alv_disp_screen TO li_alv_disp_screen.
            CLEAR st_alv_disp_screen.
          ENDIF.
        ELSE.
  ...
  IF sy-subrc EQ 0.
    ...
    LOOP AT li_excel_file_tmp INTO DATA(lst_excel).
      CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
        EXPORTING
          input  = lst_excel-matnr
        IMPORTING
          output = st_excel_file-matnr.
      st_excel_file-lang = lst_excel-lang.
      st_excel_file-eal = lst_excel-eal.
      st_excel_file-vkorg = lst_excel-vkorg.
      st_excel_file-vtweg = lst_excel-vtweg.
...
FORM f_build_fcat .
  ...
  IF r_b_txt IS NOT INITIAL.
    PERFORM f_buildcat USING:
              lv_counter 'MATNR'            'Material Number',
              lv_counter 'LANG'             'Language'   , "Item
              lv_counter 'OLD_EAL'          'Old Text'   , "Old EAL
              lv_counter 'NEW_EAL'          'New Text'   , "New EAL
              lv_counter 'STATUS'           'Status'     ,
              lv_counter 'STATUS_MESSAGE'   'Success/Error Msg'.
  ENDIF.
  IF r_s_txt IS NOT INITIAL.
    PERFORM f_buildcat USING:
              lv_counter 'MATNR'            'Material Number',
              lv_counter 'VKORG'            'Sales Org.',
              lv_counter 'VTWEG'            'Distr. Channel',
              lv_counter 'LANG'             'Language'   , "Item
              lv_counter 'OLD_EAL'          'Old Text'   , "Old EAL
              lv_counter 'NEW_EAL'          'New Text'   , "New EAL
              lv_counter 'STATUS'           'Status'     ,
...
FORM f_display_message  USING    p_lv_job_name.
  ...
  CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
    EXPORTING
      i_callback_program = sy-repid
      i_grid_title       = 'Material Text update' "text-075
      it_fieldcat        = li_fcat
...
FORM build_batch_log_data .
  REFRESH:i_attach.
*- For Final Records
  CONCATENATE 'Material Number' 'Language' 'OLD EAL' 'NEW EAL' 'Status' 'Success/Error Msg'
...
*- For Final Records
    CONCATENATE  st_alv_disp_screen-matnr        st_alv_disp_screen-lang
...
**----------------------------------------------------------------------*
FORM f_get_mat_text .
  DATA : st_read_text_rpt  TYPE TABLE OF tline,
         lst_read_text_rpt TYPE tline.
  CONSTANTS : c_en TYPE char2 VALUE 'EN',
              c_de TYPE char2 VALUE 'DE'.
  REFRESH:li_mara.
  SELECT matnr FROM mara
         INTO TABLE li_mara
         WHERE matnr IN p_matnr.
  IF li_mara[] IS NOT INITIAL.
    LOOP AT li_mara INTO DATA(st_mara_text).
      CLEAR st_header.
      st_alv_disp_screen_text-matnr = st_mara_text-matnr.
      st_header-tdobject = p_object.
      st_header-tdname = st_mara_text-matnr.
      st_header-tdid = p_id.
      st_header-tdspras = c_en."st_excel_file-lang.
      CALL FUNCTION 'READ_TEXT'
        EXPORTING
          client                  = sy-mandt
          id                      = st_header-tdid
        ...
        EXCEPTIONS
          ...
          wrong_access_to_archive = 7
          OTHERS                  = 8.
      IF sy-subrc EQ  0 AND st_read_text_rpt[] IS NOT INITIAL.
        READ TABLE st_read_text_rpt INTO lst_read_text_rpt INDEX 1.
        st_alv_disp_screen_text-old_eal = lst_read_text_rpt-tdline.
        st_alv_disp_screen_text-lang = c_en.
        APPEND st_alv_disp_screen_text TO li_alv_disp_screen_text.
      ENDIF.
      st_header-tdspras = c_de."st_excel_file-lang.
      CLEAR : st_read_text_rpt.
      CALL FUNCTION 'READ_TEXT'
        EXPORTING
          client                  = sy-mandt
          id                      = st_header-tdid
      ...
      IF sy-subrc EQ  0 AND st_read_text_rpt[] IS NOT INITIAL.
        READ TABLE st_read_text_rpt INTO lst_read_text_rpt INDEX 1.
        st_alv_disp_screen_text-old_eal = lst_read_text_rpt-tdline.
        st_alv_disp_screen_text-lang = c_de.
        APPEND st_alv_disp_screen_text TO li_alv_disp_screen_text.
      ENDIF.
      CLEAR st_alv_disp_screen_text.
    ENDLOOP.
  ELSE.
    MESSAGE e000(zrtr_r1b) WITH 'Given Selection does not have entries in SAP system'. " & & & & "Job could not be created
  ENDIF.
  IF  li_alv_disp_screen_text[] IS NOT INITIAL.
    PERFORM f_alv_rpt_text.
  ENDIF.
ENDFORM.
**&---------------------------------------------------------------------*
**&      Form  F_ALV_RPT_TEXT
...
FORM f_alv_rpt_text .
  ...
  CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
    EXPORTING
      ...
      it_fieldcat              = li_fcat_out
    TABLES
      t_outtab                 = li_alv_disp_screen_text
    EXCEPTIONS
      program_error            = 1
      OTHERS                   = 2.
  IF sy-subrc <> 0.
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM f_build_fcat_text .
  REFRESH li_fcat_out.
  DATA: lv_counter TYPE sycucol VALUE 1. " Counter of type Integers
  PERFORM f_buildcat_text USING:
            lv_counter 'MATNR'            'Material Number',
            lv_counter 'LANG'             'Language'   , "Item
            lv_counter 'OLD_EAL'          'Text'   . "Old EAL
ENDFORM.
*&-----------------------------------*
*&      Form  F_BUILDCAT
*&-----------------------------------
...
*      ->P_TEXT_001  text
*-----------------------------------*
FORM f_buildcat_text  USING  fpp_col   TYPE sycucol   " Horizontal Cursor Position
                        fpp_fld   TYPE fieldname " Field Name
                        fpp_title TYPE itex132.  " Text Symbol length 132
  CONSTANTS :           lcc_tabname       TYPE tabname   VALUE 'I_OUTPUT_X'. " Table Name
  st_fcat_out-col_pos      = fpp_col + 1.
  st_fcat_out-lowercase    = abap_true.
  st_fcat_out-fieldname    = fpp_fld.
  st_fcat_out-tabname      = lcc_tabname. "'I_OUTPUT_X'.
  st_fcat_out-seltext_m    = fpp_title.
  IF fpp_fld = 'MATNR'.
    st_fcat_out-no_zero      = abap_true.
  ENDIF.
  APPEND st_fcat_out TO li_fcat_out.
