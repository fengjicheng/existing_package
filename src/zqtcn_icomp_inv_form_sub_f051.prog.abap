*&---------------------------------------------------------------------*
*& Report  ZQTCN_ICOMP_INV_FORM_SUB_F051
*&---------------------------------------------------------------------*
* PROGRAM NAME:  ZQTCR_ICOMP_INV_FORM_F051
* PROGRAM DESCRIPTION: This driver program copied from F042 driver program ZQTCR_INVOICE_FORM_F024
*                      to tigger the TBT for intercompany invoice.
* DEVELOPER: Nagireddy Modugu
* CREATION DATE: 07/23/2019
* OBJECT ID: F051
* TRANSPORT NUMBER(S):  ED2K915751
*----------------------------------------------------------------------*
*----------------------------------------------------------------------*
* REVISION HISTORY-----------------------------------------------------*
* REVISION NO:
...
* Retrieve prepaid amount
  ...
  IF v_output_typ EQ v_outputyp_idr.
    PERFORM f_populate_layout_idr.
...
FORM f_get_constant_values  CHANGING fp_r_inv           TYPE tt_billtype " Billing Type
                                     ...
                                     fp_v_outputyp_idr  TYPE sna_kschl   " Message type
...
*   Loop through constant table to retrieve constant values
    LOOP AT li_constant INTO DATA(lst_constant).
      ...
      IF lst_constant-param1 EQ  lc_idr_type.
        fp_v_outputyp_idr = lst_constant-low.
      ENDIF. " IF lst_constant-param1 EQ lc_tbt_type
  ...
  ELSE. " ELSE -> IF sy-subrc <> 0
    TRY .
      ...
      CATCH cx_fp_api_internal INTO lr_err_int.
        ...
        LEAVE LIST-PROCESSING.
    ENDTRY.
    IF fp_v_formname EQ c_idr_form.

      IF v_seller_reg IS NOT INITIAL.
        CONCATENATE 'Wiley Tax ID :'(003) v_seller_reg INTO v_seller_reg SEPARATED BY space.
      ENDIF.
...
* Data declaration
  DATA : li_lines       TYPE STANDARD TABLE OF tline, "Lines of text read
         ...
         lst_line       TYPE tline,                   " SAPscript: Text Lines
         lst_lines      TYPE tline,                   " SAPscript: Text Lines
         lv_remit_to    TYPE char30,
         lv_bank_detail TYPE char30.
...
* Constant Declaration
  CONSTANTS : lc_css_remit_to     TYPE char30     VALUE 'ZQTC_F042_CSS_REMIT_TO_',      " Remit_to of type CHAR20
              ...
              lc_idr_remit_to     TYPE char30     VALUE 'ZQTC_F042_IDR_REMIT_TO_',      " Remit_to of type CHAR20
...
*              lc_tbt_agency_crd1   TYPE char30     VALUE 'ZQTC_CREDIT_CARD_AGENCY_', " Crd_card_det of type CHAR22
              ...
              lc_idr_bank_det     TYPE char30     VALUE 'ZQTC_F042_IDR_BANK_DET_',   " Bank details
...
*              lc_soc_bank_det     TYPE char30     VALUE 'ZQTC_F042_SOC_BANK_DET_',    " Bank details
              lc_soc_bank_det     TYPE char30     VALUE 'ZQTC_F027_BANKING1_', " Bank details
...
* Retrieve all the standard text names
  ...
  IF  v_output_typ EQ v_outputyp_idr .
    CLEAR : lv_remit_to.
    IF v_output_typ = v_outputyp_tbt.
      lv_remit_to = lc_tbt_remit_to.
    ELSEIF v_output_typ = v_outputyp_idr.
      lv_remit_to = lc_idr_remit_to.
    ENDIF.

* Fetch Remit to text
    CONCATENATE lv_remit_to
                fp_st_vbrk-bukrs
                lc_undscr
                fp_st_vbrk-waerk
           INTO fp_v_remit_to.
...
***End of Change: SRBOSE on 15-Mar-2018: ERP_6599: TR: ED2K911376
    CLEAR : lv_bank_detail.
    IF v_output_typ = v_outputyp_tbt.
      lv_bank_detail = lc_tbt_bank_det.
    ELSEIF v_output_typ = v_outputyp_idr.
      lv_bank_detail = lc_idr_bank_det.
    ENDIF.

* Populate Bank Details
    CONCATENATE lv_bank_detail
                fp_st_vbrk-bukrs
                lc_undscr
                fp_st_vbrk-waerk
...
* Retrieve Tax/VAT values and add with document currency value
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        CONDENSE lv_text.
        CONCATENATE lv_text fp_st_header-doc_currency INTO fp_v_vat SEPARATED BY space.
      ENDIF. " IF sy-subrc EQ 0
    ENDIF. " IF sy-subrc EQ 0
...
* Fetch sub-total text from text module
    ...
    LOOP AT i_txtmodule INTO lst_line.
      CONCATENATE lst_line-tdline fp_st_header-doc_currency INTO fp_v_subtotal SEPARATED BY space.
...
* Fetch Total due text from text module
    ...
    LOOP AT i_txtmodule INTO lst_line.
      CONCATENATE lst_line-tdline fp_st_header-doc_currency INTO fp_v_totaldue SEPARATED BY space.
...
* Fetch prepaid amount text from text module
    ...
    LOOP AT i_txtmodule INTO lst_line.
      CONCATENATE lst_line-tdline fp_st_header-doc_currency INTO fp_v_prepaidamt SEPARATED BY space.
...
* Fetch Shipping text from text module
    ...
    LOOP AT i_txtmodule INTO lst_line.
      CONCATENATE lst_line-tdline fp_st_header-doc_currency INTO fp_v_shipping SEPARATED BY space.
...
*   If company code Australia or Singapore, then populate tax credit as title
    IF v_output_typ EQ v_outputyp_idr.
      PERFORM f_read_text    USING c_tax_invoice_idr
                        CHANGING fp_v_accmngd_title.
    ELSE.
      IF fp_st_header-comp_code IN r_country.
...
***Year
        ...
        IF v_start_year IS NOT INITIAL.
          CONCATENATE lv_year v_start_year INTO lst_final_tbt-subs_type SEPARATED BY lc_colon.
          CONDENSE lst_final_tbt-subs_type.
...
*         End   of DEL:ERP-7124:WROY:16-Mar-2018:ED2K911376
            IF sy-subrc EQ 0.
              lv_flag = abap_true.
              CONCATENATE lst_society-society_name lv_membership v_start_year INTO lst_final_soc-description
              SEPARATED BY space.
            ENDIF. " IF sy-subrc EQ 0
...
***Year
        ...
        IF v_start_year IS NOT INITIAL.
          CONCATENATE lv_year v_start_year INTO lst_final_soc-description SEPARATED BY lc_colon.
          CONDENSE lst_final_soc-description.
...
*Call FM to get the list of PDF attachments for the particular material
*and attachment name ending with KDKG2
      ...
      IF nast-nacha EQ lc_nacha_01.
        CLEAR lst_output.
        lst_output-attachment_name = lv_subject.
        lst_output-pdf_stream = st_formoutput-pdf.
