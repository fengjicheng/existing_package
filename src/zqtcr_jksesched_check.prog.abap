*----------------------------------------------------------------------*
* PROGRAM NAME:        ZQTCR_JKSESCHED_CHECK
* PROGRAM DESCRIPTION: As per OSS Message 62596/2019 (REPORT zjsd_oss_62596_2019)
...
* c) JKSECONTRINDEX not available, VBAP available but AUART is not I-SM
*                   AUART (ism_contract <> '02')
...
*    a) No change document data available WRITE: / 'Order exists, but not
*                   valid...a)'
*    b)  no VENDDAT/VBEGDAT field changes available WRITE: / 'Order exists,
*                   but not valid...b)'
*    c) generated orde not found             WRITE: / 'Order exists, but
*                   not valid...c)'
...
* OBJECT ID:           Z Program per OSS Message 62596/2019
* TRANSPORT NUMBER(S): ED2K916533
*----------------------------------------------------------------------*
* Revision History-----------------------------------------------------*
* Revision No:
* Reference No:
...
* VBAP entrie not available -> probaby deleted
          IF p_sum IS INITIAL.
            WRITE: / 'b) VBAP missing:', <fs_jksesched>-vbeln, <fs_jksesched>-posnr,
            <fs_jksesched>-issue, <fs_jksesched>-sequence.
          ENDIF.
...
* no IS-Media Series contract
              IF p_sum IS INITIAL.
                WRITE: / 'c) no IS-M document type', <fs_jksesched>-vbeln, <fs_jksesched>-posnr,
                <fs_jksesched>-issue, <fs_jksesched>-sequence, ls_tvak-auart.
              ENDIF.
              APPEND <fs_jksesched> TO lt_jksesched_del.
            ...
            ELSEIF ls_tvak-ism_contract = '02' AND
                   ls_tvak-trvog <> '4'.
              IF p_sum IS INITIAL.
                WRITE: / 'd) IS-M document type, but no contract', <fs_jksesched>-vbeln,
                <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence, ls_tvak-auart.
              ENDIF.
              APPEND <fs_jksesched> TO lt_jksesched_del.
              lv_counter_auart_ism_missing =  lv_counter_auart_ism_missing + 1.
            ELSE.
              IF p_sum IS INITIAL.
                WRITE: / 'e) check contract type', <fs_jksesched>-vbeln, <fs_jksesched>-posnr,
                <fs_jksesched>-issue, <fs_jksesched>-sequence, ls_tvak-auart.
              ENDIF.
              APPEND <fs_jksesched> TO lt_jksesched_del.
...
* No change document data available
              IF p_sum IS INITIAL.
                WRITE: / 'Order exists, but not valid...a)', <fs_jksesched>-vbeln,
                <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
              ENDIF.
...
* no VENDDAT/VBEGDAT field changes available
                IF p_sum IS INITIAL.
                  WRITE: / 'Order exists, but not valid...b)', <fs_jksesched>-vbeln,
                  <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
                ENDIF.
                lv_counter_order_exists = lv_counter_order_exists + 1.
                CONTINUE.
...
* Changedocument available, validity was changed, but still not valid
                  IF p_sum IS INITIAL.
                    WRITE: / 'Order exists, but not valid...e)', <fs_jksesched>-vbeln,
                    <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
                  ENDIF.
                  lv_counter_order_exists =  lv_counter_order_exists + 1.
                  CONTINUE.
...
* Changedocument available, but didn't influence the validity
                IF p_sum IS INITIAL.
                  WRITE: / 'Order exists, but not valid...f)', <fs_jksesched>-vbeln,
                  <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
                ENDIF.
                lv_counter_order_exists = lv_counter_order_exists + 1.
                CONTINUE.
...
* generated order not found
            IF p_sum IS INITIAL.
              WRITE: / 'Order exists, but not valid...c)', <fs_jksesched>-vbeln,
              <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
            ENDIF.
            lv_counter_order_exists = lv_counter_order_exists + 1.
            CONTINUE.
...
* no order found or more than one order found
          IF p_sum IS INITIAL.
            WRITE: / 'Order exists, but not valid... d)', <fs_jksesched>-vbeln,
            <fs_jksesched>-posnr, <fs_jksesched>-issue, <fs_jksesched>-sequence.
          ENDIF.
          lv_counter_order_exists = lv_counter_order_exists + 1.
          CONTINUE.
      ...
      IF lv_valid = abap_false.
        ...
        IF p_sum IS INITIAL.
          WRITE: / <fs_jksesched>-vbeln, <fs_jksesched>-posnr,
          <fs_jksesched>-issue, <fs_jksesched>-sequence.
        ENDIF.
      ENDIF.
  ...
  WRITE: / '   VBAP available but AUART is no I-SM AUART (ism_contract<>02)',
  lv_counter_auart_noism_missing.
  WRITE: / 'd) JKSECONTRINDEX not available'.
  WRITE: / '   VBAP available but AUART is I-SM AUART (ism_contract= 02,TRVOG<>4)',
  lv_counter_auart_ism_missing.
