...
*----------------------------------------------------------------------*
REPORT zqtcn_chg_ord_item_text_batch.
TABLES:adr6.
* Type Declaration
TYPES : BEGIN OF ty_excel_file,
          vbeln TYPE vbak-vbeln,  " Leg Ref Number
...
PARAMETERS: p_object TYPE thead-tdobject DEFAULT 'VBBP' NO-DISPLAY.
PARAMETERS: p_id TYPE thead-tdid DEFAULT'0012' NO-DISPLAY.
...
FORM read_from_app .
  ...
  DO.
    ...
    IF sy-subrc EQ 0.
      SPLIT lv_string AT c_tab      INTO st_excel_file-vbeln
                                    st_excel_file-posnr
                                    st_excel_file-eal.
...
FORM order_upd .
  ...
  i_mail[] = p_mail[].
*  p_file =
  PERFORM f_bapi_update_with_new_value IN PROGRAM zqtcn_change_order_item_text USING p_user
                                                                                p_job
                                                                                p_lang
                                                                                p_object
                                                                                p_id
