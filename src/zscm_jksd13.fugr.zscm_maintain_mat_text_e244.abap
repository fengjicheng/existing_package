FUNCTION zscm_maintain_mat_text_e244.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
  ...
  IF <lv_preq_date> IS ASSIGNED AND
    ...
    IF <lfi_outtab> IS ASSIGNED AND <lfi_outtab> IS NOT INITIAL.
      ...
      IF sy-subrc <> 0.
        ...
        EXIT.
      ENDIF.
      " Call transaction JP28 to maintain Material text in DE
      ...
      IF sy-subrc = 0.
        li_bdcdata_tab = VALUE #(
                ...
                ( fnam = 'RMMG1-MATNR' fval = lst_outtab-matnr ) ).

*        lst_opt-dismode = 'A'.
*        lst_opt-defsize = abap_true.

        TRY.
            CALL TRANSACTION 'JP28' USING li_bdcdata_tab. " OPTIONS FROM lst_opt.
  ...
  ELSE.
    MESSAGE '"PR-DE Language" is only applicable for Purchase Req. Date'(005) TYPE 'I'.
    EXIT.
  ENDIF.
