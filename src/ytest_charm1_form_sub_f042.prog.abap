...
*----------------------------------------------------------------------*
* REVISION HISTORY-----------------------------------------------------*
* REVISION NO :  OTCM-60474 / Indian Agent form changes
* REFERENCE NO:  ED2K926474
* DEVELOPER   :  Bsaki(BHarat Saki)
* DATE        :  03/31/2022
...
* BOC OTCM_60474:Indian Agent ZACS_ZACD Inv Form Changes by bsaki 31/03/2022

  DATA : lv_po_type      TYPE vbak-bsark, lv_sales_office TYPE vbak-vkbur.
  SELECT SINGLE aubel FROM vbrp INTO @DATA(lv_so) WHERE vbeln = @st_vbrk-vbeln and aubel ne @space .
  IF lv_so IS NOT INITIAL.
    CLEAR : lv_sales_office , lv_po_type.
    SELECT SINGLE vkbur bsark FROM vbak INTO (lv_sales_office , lv_po_type) WHERE vbeln = lv_so.
    IF lv_sales_office = '0080' AND lv_po_type = '0161' and nast-kschl = 'ZTBT'.
      CONCATENATE lc_cust_service
               lv_sales_office '_'
               lv_po_type
        INTO   fp_v_cust_service_det.
    ...
    CLEAR : lv_so.
  ENDIF.
*EOC OTCM_60474:Indian Agent ZACS_ZACD Inv Form Changes by bsaki 31/03/2022
**********************************************************************
