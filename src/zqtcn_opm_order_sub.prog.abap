...
*      <-P_I_FINAL  text
*-----------------------------------*
FORM f_convert_new_zopm_ord_excel USING fp_p_file  TYPE rlgrap-filename " Local file for upload/download 5
                               CHANGING fp_i_final TYPE tt_excel_zopm.
...
*  <-  p2        text
*-----------------------------------*
FORM f_display_new_zopm_ord_alv .  "6
  DATA:lst_final TYPE ty_excel_zopm.
  REFRESH i_fcat_out.
...
* fill up the ALV table
  LOOP AT i_final_zopm INTO st_final_x.
    CLEAR: st_output_x.
...
*      WHERE ( kunnr = @li_output-kunnr_sp OR kunnr = @li_output-kunnr_we OR kunnr = @li_output-partner )
      WHERE vkorg = @v_c108_sorg
      AND vtweg = @v_c108_dist
...
*     FOR ALL ENTRIES IN @li_output
     WHERE kappl EQ 'V'.
*     AND   ( kschl EQ @li_output-kschl OR kschl EQ @li_output-kschl2 OR kschl EQ @v_c108_cond ).
      IF sy-subrc IS INITIAL.
...
**   Whenever we see new header entry, we refresh the tables
    IF lst_output_dummy-auart  EQ c_zopm.
      CLEAR: lst_output_x.
...
*====================================================================*
      IF lst_contract_hdrin IS INITIAL.
        ...
        lst_contract_hdrin-sales_org  = v_c108_sorg.
        lst_contract_hdrin-distr_chan = v_c108_dist.
        ...
        lst_contchg_hdr-sales_org  = v_c108_sorg.
        lst_contchg_hdr-distr_chan = v_c108_dist.
...
*COndition Type 1
        IF lst_output_dummy-kbetr > 0.
          ...
          lst_contract_cond-cond_type  = v_c108_cond.

          READ TABLE li_cond_class ASSIGNING FIELD-SYMBOL(<lfs_cond>) "  class
          WITH KEY kschl = v_c108_cond
          BINARY SEARCH.
...
*   Before a new header record/ last entry - Create contract
    ...
    ELSE. " ELSE -> IF lv_index EQ lines( li_create_contract )
      IF li_create_contract[ ( lv_index + 1 ) ]-auart EQ c_zopm.
...
FORM f_send_email_move_file .
  ...
  IF p_a_file IS NOT INITIAL.
    ...
    IF sy-subrc = 0.
      REPLACE ALL OCCURRENCES OF '/C108/in' IN lv_path WITH '/C108/err'.
    ELSE. " ELSE -> IF sy-subrc = 0
      REPLACE ALL OCCURRENCES OF '/C108/in' IN lv_path WITH '/C108/prc'.
    ENDIF. " IF sy-subrc = 0
...
*** Local Constant Declaration
  CONSTANTS: lc_pipe       TYPE char1 VALUE '|',        " Tab of type Character
             ...
             lc_job_name   TYPE btcjob VALUE 'ZORDER_ZOPM'. " Background job name
