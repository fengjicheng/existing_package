"Name: \FU:ISM_CIC_CUSTENV_SDCONTRACTS\SE:END\EI
...
* PROGRAM DESCRIPTION: Sort the Contracts by creation date
* DEVELOPER: AMOHAMMED
...
* Local Constant Declaration
  CONSTANTS:
    lc_sno_e229_001   TYPE zsno    VALUE '001',   "Serial Number
...
* To check enhancement is active or not
  CALL FUNCTION 'ZCA_ENH_CONTROL'
    EXPORTING
      ...
      im_ser_num     = lc_sno_e229_001    "Serial Number (001)
    IMPORTING
      ex_active_flag = lv_actv_flag_e229. "Active / Inactive Flag

  IF lv_actv_flag_e229 EQ abap_true. "Check for Active Flag
    IF ex_sdcontract_keytab IS NOT INITIAL.
      SELECT vbeln, erdat, auart                            " Fetch the Contract creation date
        FROM vbak
        INTO TABLE @DATA(lt_vbak)
        FOR ALL ENTRIES IN @ex_sdcontract_keytab
        WHERE vbeln = @ex_sdcontract_keytab-vbeln.
      IF lt_vbak IS NOT INITIAL.
        SORT lt_vbak by auart erdat DESCENDING.             " Sort by document type and creation date
        REFRESH ex_sdcontract_keytab.                       " Delete existing data
        MOVE-CORRESPONDING lt_vbak TO ex_sdcontract_keytab. " Restore with required data
